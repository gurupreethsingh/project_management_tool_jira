...
export default function ChatBot({ askApi = ASK_API }) {
  ...
  async function sendQuestion(question) {
    ...
    try {
      ...
      interactionId = startRes?.data?.data?._id || null;
    } catch (e) {
      console.warn("start-interaction failed:", e?.response?.data || e.message);
    }
    ...
    try {
      ...
      aiMeta = {
        ...
        suggestions: Array.isArray(aiData.suggestions)
          ? aiData.suggestions
          : [],
        ragSources: Array.isArray(aiData.rag_sources) ? aiData.rag_sources : [],
        conversationId: aiData.conversation_id || "",
        sessionId: aiData.session_id || sid,
      };
    ...
    try {
      const payloadCommon = {
        responseText: aiText || "No response from model.",
        responseStatus: aiText ? "good" : "no_response",
        model: aiMeta.source || "rag-memory-model",
        modelVersion: "v1",
        meta: {
          confidence: aiMeta.confidence,
          ragSources: aiMeta.ragSources,
          conversationId: aiMeta.conversationId,
          sessionId: aiMeta.sessionId,
        },
      };
          ...
          <div ref={listRef} className="flex-1 overflow-y-auto p-3 bg-white">
            {messages.map((m) => (
              <div key={m.id}>
                ...
                {m.role === "ai" &&
                  (m.source || typeof m.confidence === "number") && (
                    <div className="ml-10 -mt-2 mb-2 text-[10px] text-gray-500 flex items-center gap-2 flex-wrap">
                      {m.source ? <span>source: {m.source}</span> : null}
                      ...
                      {Array.isArray(m.ragSources) &&
                      m.ragSources.length > 0 ? (
                        <span>rag: {m.ragSources[0]?.source}</span>
                      ) : null}
                    </div>
                  )}
