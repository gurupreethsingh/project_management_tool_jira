// src/components/ai_components/TextCodeGenAssistantShell.jsx
...
// Text-to-code Node API base
const CODE_BASE = `${API}/api/text-code`;
const ASK_CODE = `${CODE_BASE}/generate`;
const INFO_CODE = `${CODE_BASE}/model-info`;
const RELOAD_CODE = `${CODE_BASE}/reload`;

// Session & localStorage helpers
const SID_KEY = "code_gen_workspace_sid_v1";
...
// We only preview in iframe when it's HTML/JSX
...
function lsKey(scope, suffix) {
  return `code_${scope}_${suffix}_v1`;
}

export default function TextCodeGenAssistantShell({
  title = "Text to Code Generator",
  scope = "text-code-gen",
  placeholder = "Describe the code you want (language, inputs, outputs, constraints)…",
  starterPrompts = [],
}) {
    ...
    return [
      {
        id: "welcome",
        role: "ai",
        text: `💻 Welcome to ${title}. Describe the behavior, language, and structure, and I’ll generate clean code for you.`,
        time: Date.now(),
      },
    ];
  });
  ...
  // Initial model info fetch
  useEffect(() => {
    (async () => {
      try {
        const r = await axios.get(INFO_CODE, { headers, timeout: 10000 });
    ...
    setMessages([
      {
        id: "welcome",
        role: "ai",
        text: `🆕 New ${title} session. What code should I write for you?`,
        time: Date.now(),
      },
    ]);
    setInput("");
  ...
  function downloadCode(code, filename = "generated_code_snippet.txt") {
  ...
  async function reloadModel() {
    try {
      await axios.post(RELOAD_CODE, {}, { headers, timeout: 60000 });
      const info = await axios.get(INFO_CODE, { headers, timeout: 10000 });
  ...
  async function send() {
    ...
    let code = "";
    try {
      // First try with retrieval enabled (if backend supports)
      let payload = { task, max_new_tokens: 512, use_retrieval: true };
      let resp = await axios.post(ASK_CODE, payload, {
        headers,
        timeout: 120000,
      });
      ...
      if (badCode) {
        // Second pass: force generation, no retrieval
        payload = { task, max_new_tokens: 512, use_retrieval: false };
        resp = await axios.post(ASK_CODE, payload, {
          headers,
          timeout: 120000,
        });
    ...
    // Only preview when code is HTML/JSX
    ...
    <div className="w-full min-h-[calc(100vh-8rem)] sm:min-h-[calc(100vh-10rem)]">
      ...
      <div className="mx-auto max-w-7xl grid md:grid-cols-[280px,1fr]">
            ...
            <ul className="space-y-1 max-h-[70vh] md:max-h-[calc(100vh-18rem)] overflow-y-auto">
              {topics.length === 0 && (
                <li className="text-xs text-gray-500">
                  No topics yet. Describe some code to start!
                </li>
              )}
              {topics.map((t) => (
                <li
        ...
        <main className="min-h-[70vh] flex flex-col">
          ...
          <div className="hidden md:flex items-center justify-between px-6 py-4 border-b bg-white sticky top-0 z-10">
            <div className="flex items-center gap-3">
              ...
              <button
                onClick={async () => {
                  try {
                    const r = await axios.get(INFO_CODE, {
                      headers,
                      timeout: 10000,
                    });
                    setModelInfo(r?.data || null);
            ...
            <div className="mx-auto max-w-4xl">
                  ...
                  {m.role === "ai" && m.text && (
                    <div className="mt-2 flex flex-wrap gap-2 text-xs">
                      ...
                      <button
                        ...
                        className="inline-flex items-center gap-1 border rounded px-2 py-1 hover:bg-gray-50"
                        title="Open HTML preview below"
                        disabled={!looksLikeHtmlOrJsx(m.text)}
                      >
                        <FiExternalLink /> Preview (HTML)
                      </button>
                      <button
                        onClick={() => downloadCode(m.text)}
                        className="inline-flex items-center gap-1 border rounded px-2 py-1 hover:bg-gray-50"
              ...
              {busy && (
                <div className="flex items-center gap-2 text-sm text-gray-500">
                  <FiLoader className="animate-spin" /> generating code…
                </div>
              )}
              {error && (
          ...
          {/* Live preview (only meaningful for HTML/JSX output) */}
          {previewUrl && (
            <div className="border-t bg-gray-50">
              <div className="mx-auto max-w-5xl px-4 md:px-6 py-3">
                <div className="flex items-center justify-between mb-2">
                  <div className="text-xs text-gray-600">
                    Live HTML Preview (for code that returns full HTML)
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button
                      onClick={() => {
                        if (lastCode && looksLikeHtmlOrJsx(lastCode)) {
                          openPreview(lastCode);
                        }
                      }}
                      className="inline-flex items-center gap-1 border rounded px-2 py-1 text-xs bg-white hover:bg-gray-50"
                    >
