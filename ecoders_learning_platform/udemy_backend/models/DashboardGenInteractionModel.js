...
// Keep status & content types aligned with UiGen and controller usage
...
const DashboardGenInteractionSchema = new Schema(
  {
    user: {
      type: Types.ObjectId,
      ref: "User",
  ...
  {
    timestamps: true,
    collection: "dashboard_generator_interactions",
  }
);

// Auto-calc respondedAt / latency if missing
DashboardGenInteractionSchema.pre("save", function (next) {
  try {
...
// Hash IP exactly like UiGen, but with its own salt key
DashboardGenInteractionSchema.statics.hashIp = function (ip) {
  if (!ip) return undefined;
  const salt = process.env.DASH_IP_SALT || "rotate-this-dashboard-salt";
  return crypto.createHash("sha256").update(`${salt}:${ip}`).digest("hex");
};

const DashboardGenInteraction = mongoose.model(
