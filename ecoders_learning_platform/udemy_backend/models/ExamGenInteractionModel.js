// api/models/ExamGenInteractionModel.js

const mongoose = require("mongoose");
const crypto = require("crypto");
const { Schema, Types } = mongoose;

// Keep status & content types aligned with UiGen / DashboardGen and controller usage
...
const ResponseSchema = new Schema(
  {
    // For exam generator we store plain text exam paper
    paper: { type: String, default: "" },

    contentType: {
      type: String,
      enum: CONTENT_TYPE,
      default: "text", // exam paper is plain text by default
    },
    status: {
      type: String,
      enum: STATUS,
...
const ExamGenInteractionSchema = new Schema(
  {
    user: {
      type: Types.ObjectId,
      ref: "User",
  ...
  {
    timestamps: true,
    collection: "exam_generator_interactions",
  }
);

// Auto-calc respondedAt / latency if missing
ExamGenInteractionSchema.pre("save", function (next) {
  try {
    if (this.response && this.response.paper && !this.respondedAt) {
...
// Hash IP exactly like UiGen/DashboardGen, but with its own salt key
ExamGenInteractionSchema.statics.hashIp = function (ip) {
  if (!ip) return undefined;
  const salt = process.env.EXAM_IP_SALT || "rotate-this-exam-salt";
  return crypto.createHash("sha256").update(`${salt}:${ip}`).digest("hex");
};

const ExamGenInteraction = mongoose.model(
  "ExamGenInteraction",
  ExamGenInteractionSchema
);
