// api/models/UiGenInteractionModel.js
...
const ResponseSchema = new Schema(
  {
    ...
    contentType: { type: String, enum: CONTENT_TYPE, default: "html" },
    status: { type: String, enum: STATUS, default: "ok", index: true },
    source: { type: String, trim: true }, // model | retrieval | low_confidence
...
const UiGenInteractionSchema = new Schema(
  {
  ...
  {
    timestamps: true,
    collection: "ui_generator_interactions",
  }
);

UiGenInteractionSchema.pre("save", function (next) {
  try {
    if (this.response && this.response.code && !this.respondedAt) {
...
UiGenInteractionSchema.statics.hashIp = function (ip) {
  if (!ip) return undefined;
  const salt = process.env.UI_IP_SALT || "rotate-this-ui-salt";
  return crypto.createHash("sha256").update(`${salt}:${ip}`).digest("hex");
};

const UiGenInteraction = mongoose.model(
  "UiGenInteraction",
  UiGenInteractionSchema
);
module.exports = { UiGenInteraction, STATUS, CHANNEL, CONTENT_TYPE };
