...
// const writeGuestCart = (items = [], emitGuestEvent = false) => {
//   try {
...
//           detail: { items, source: "homepage", suppressToast: true },
//         }),
//       );
//     }
//   } catch (error) {
...
// const addProductToGuestCart = (product, options = {}) => {
...
//   if (existingIndex >= 0) {
//     updatedQty = getSafeNumber(guestItems[existingIndex]?.quantity, 1) + 1;
...
//       selling_price: getSafeNumber(
//         product?.selling_price || product?.display_price,
//         0,
//       ),
//       display_price: getSafeNumber(product?.display_price, 0),
//       brand: product?.brand || "",
...
//   writeGuestCart(guestItems, Boolean(options?.emitGuestEvent));
//   return updatedQty;
// };
...
// /* =========================================================
//    AUTOMATION ATTRIBUTES FOR SELENIUM
// ========================================================= */
// // ✅ Hidden automation attributes for Selenium verification.
// // These details are not visible in the UI, but Selenium can read them using getAttribute().
...
//     if (typeof value.name === "string") return value.name.trim().toUpperCase();
//     if (typeof value.title === "string")
//       return value.title.trim().toUpperCase();
//     if (typeof value.category_name === "string") {
//       return value.category_name.trim().toUpperCase();
//     }
//     if (typeof value.subcategory_name === "string") {
//       return value.subcategory_name.trim().toUpperCase();
//     }
//   }
//   return "";
// };
...
//   getId(product?.category?._id || product?.category);

// const getSubcategoryIdFromProduct = (product) =>
//   getId(product?.subcategory?._id || product?.subcategory);
...
// const getSubcategoryNameFromProduct = (product) =>
//   safeUpper(
...
//     if (value.subcategory_name) return String(value.subcategory_name);
...
// const getProductAutomationAttrs = (product, source = "homepage", index = 0) => {
//   const sellingPrice =
...
//     "data-product-dimensions": toDataValue(product?.dimensions),
...
// // ✅ Hidden product details block for Selenium verification.
// // This is hidden from UI, but Selenium can read textContent or each data-field span.
// const HiddenProductDetails = ({ product, source = "homepage", index = 0 }) => {
//   const attrs = getProductAutomationAttrs(product, source, index);
//   const fields = [
//     "product-id",
//     "product-name",
//     "product-slug",
...
//     "product-max-purchase-qty",
//     "product-delivery-time-estimate",
...
//     <div
//       aria-hidden="true"
//       className="hidden product-hidden-details"
//       data-testid="homepage-product-hidden-details"
//       {...attrs}
//     >
//       {fields.map((field) => (
...
//   const syncCartCountsFromLocalAndContext = useCallback(() => {
//     const token = getStoredToken();

//     // Guest cart is stored in localStorage. No backend call is needed.
//     if (!token) {
//       setCartCounts(getGuestCartCountMap());
//       return;
//     }

//     // Logged-in cart should come from CartContext, exactly like Shop page.
//     if (!Array.isArray(cartItems)) {
//       setCartCounts({});
//       return;
//     }

//     const countsMap = cartItems.reduce((acc, cartItem) => {
//       const productId =
...
//     fetchData();
//     // IMPORTANT:
//     // Do NOT sync cart inside this product-fetching effect.
//     // Cart updates change CartContext, and if this effect depends on cart sync,
//     // the homepage can refetch/re-render repeatedly.
//   }, []);

//   useEffect(() => {
//     window.addEventListener(
//       GUEST_CART_EVENT,
//       syncCartCountsFromLocalAndContext,
//     );
//     window.addEventListener(
//       CART_UPDATED_EVENT,
//       syncCartCountsFromLocalAndContext,
//     );
//     window.addEventListener("storage", syncCartCountsFromLocalAndContext);
...
//         syncCartCountsFromLocalAndContext,
//       );
//       window.removeEventListener("storage", syncCartCountsFromLocalAndContext);
//     };
//   }, [syncCartCountsFromLocalAndContext]);

//   useEffect(() => {
//     syncCartCountsFromLocalAndContext();
//   }, [cartItems, syncCartCountsFromLocalAndContext]);
...
//       const productId = normalizeProductId(product);
//       if (!productId || isAddingToCartId === productId) return;

//       if (product?.availability_status === false) {
...
//         let updatedQty = Number(cartCounts[productId] || 0) + 1;

//         // ✅ Logged-in users use the same CartContext flow as Shop page.
//         // This keeps the same single toast message used in Shop.
//         if (hasToken && typeof addToCart === "function") {
//           await addToCart(product);
//         } else {
//           // ✅ Guest users use localStorage cart, then show only one react-toastify toast.
//           updatedQty = addProductToGuestCart(product);
...
//       }
//     },
//     [addToCart, cartCounts, isAddingToCartId],
//   );
...
//                 >
//                   <div className="relative">
//                     <div className="w-full h-[115px] sm:h-[130px] md:h-[140px] lg:h-[150px] bg-white flex items-center justify-center ">
//                       <img
...
//                           <span className="inline-flex items-center rounded-full bg-green-50 text-green-700 border border-green-100 px-2 py-1 text-[9.5px] sm:text-[10px] font-bold">
//                             {getAvailabilityLabel(item)}
//                           </span>
//                           <span className="inline-flex items-center rounded-full bg-orange-50 text-orange-700 border border-orange-100 px-2 py-1 text-[9.5px] sm:text-[10px] font-bold">
...
//                           disabled={isAdding}
//                           className="shrink-0 inline-flex items-center gap-1.5 rounded-xl bg-gradient-to-r from-orange-500 to-amber-500 hover:from-orange-600 hover:to-amber-600 disabled:from-orange-300 disabled:to-amber-300 text-white px-3 py-2 text-[10px] sm:text-[11px] font-extrabold transition shadow-lg shadow-orange-500/20"
//                         >
...
//                       <h4 className="text-[12px] sm:text-[12.5px] font-extrabold text-gray-900 leading-snug line-clamp-2 min-h-[2.8rem] sm:min-h-[3rem]">
//                         {item.product_name}
//                       </h4>

//                       <div className="mt-1 text-[10px] sm:text-[10.5px] text-gray-500 truncate font-semibold">
...
//                         <span className="text-[10px] sm:text-[10.5px] text-gray-500 truncate">
//                           {quantityInCart > 0
...
//                         <span
//                           className="text-[10.5px] sm:text-[11px] font-extrabold text-orange-600 hover:text-orange-700 shrink-0"
//                           onClick={(e) => {
//                             e.stopPropagation();
...
//                 </h2>
//                 <p className="hidden sm:block text-gray-600 text-[12px] sm:text-[12px] md:text-[13px] lg:text-[14px] mt-1">
//                   Smooth browsing with full product rendering.
//                 </p>
//               </div>
...
//                   {brandVisibleProducts.map((item, index) => {
//                     const productId = normalizeProductId(item);
...
//                         <HiddenProductDetails
//                           product={item}
//                           source="homepage-brand-products"
//                           index={index}
//                         />
//                         <div className="relative">
//                           <div className="w-full aspect-square bg-white flex items-center justify-center p-3 sm:p-4">
//                             <img
//                               src={resolveProductImage(item)}
//                               alt={item.product_name}
//                               className="w-full h-full object-contain"
...
//                           <h4 className="text-[12px] sm:text-[13px] font-extrabold text-gray-900 leading-snug line-clamp-2 min-h-[2.9rem] sm:min-h-[3.1rem]">
//                             {item.product_name}
//                           </h4>
...
const getProductDateValue = (product) => {
  // Featured Picks must use the product creation date first.
  // Supports your DB naming variations: created_date, createdAt, created_at.
  const rawDate =
  ...
  if (typeof rawDate === "string" && /^[0-9a-fA-F]{8}$/.test(rawDate)) {
    return parseInt(rawDate, 16) * 1000;
  }
...
const sortProductsByLatest = (products = []) =>
  [...products].sort((a, b) => getProductDateValue(b) - getProductDateValue(a));
...
const Homepage = () => {
  ...
  const [filterValue, setFilterValue] = useState("all");
  const [productSearch, setProductSearch] = useState("");
  const [visibleProductCount, setVisibleProductCount] = useState(32);
  const [isFetchingProducts, setIsFetchingProducts] = useState(true);
  const [isAddingToCartId, setIsAddingToCartId] = useState("");
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const latestProducts = sortProductsByLatest(allProducts);
        const latestFeatured = latestProducts.slice(0, 8);
        const featuredIdSet = new Set(
          latestFeatured.map((product) => normalizeProductId(product)),
        );
        ...
        for (const product of latestProducts) {
          const productId = normalizeProductId(product);
          if (featuredIdSet.has(productId)) continue;
        ...
        setFeaturedProducts(latestFeatured);
        setBrands([...brandSet]);
        setBrandedProducts(onlyBrandedProducts);
      } catch (error) {
        console.error("Homepage fetch error:", error);
  ...
  useEffect(() => {
    setVisibleProductCount(32);
  }, [filterValue, productSearch]);

  const brandVisibleProducts = useMemo(
    () => filteredBrandedProducts.slice(0, visibleProductCount),
    [filteredBrandedProducts, visibleProductCount],
  );

  const hasMoreBrandProducts =
    visibleProductCount < filteredBrandedProducts.length;
  ...
  return (
    <div className="min-h-screen relative bg-white">
      ...
      <div className="hp-font mx-auto px-3 sm:px-4 md:px-6 lg:px-10 xl:px-1">
        <section className="pt-2 sm:pt-4">
          <div className="relative overflow-hidden rounded-2xl lg:rounded-[26px] shadow-[0_30px_80px_-50px_rgba(0,0,0,0.45)]">
            <div
              id="carouselExampleDark"
          ...
          <div className="relative">
            ...
            >
              {categories.map((cat, index) => (
                <div
                  key={`${cat._id}-${index}`}
                  className="min-w-[190px] sm:min-w-[210px] md:min-w-[230px] lg:min-w-[240px] rounded-2xl overflow-hidden cursor-pointer flex-shrink-0 bg-white border border-gray-100 shadow-sm hover:shadow-md transition-transform duration-300 hover:-translate-y-1"
                  onClick={() => handleClick(cat.category_name)}
                  title={cat.category_name}
                >
                  <div className="relative">
                    <div className="w-full h-[105px] sm:h-[118px] md:h-[128px] lg:h-[136px] bg-white flex items-center justify-center">
                      <img
                        src={`${globalBackendRoute}/${cat.category_image}`}
                        alt={cat.category_name}
                  ...
                  <div className="px-3 py-2.5 border-t border-gray-100">
                    <div className="flex items-center justify-between gap-2">
                      <h3 className="text-[12px] sm:text-[13px] font-extrabold text-gray-900 truncate">
                        {cat.category_name}
                      </h3>
                      <span className="text-[10px] font-extrabold text-orange-600 shrink-0">
                        Shop →
                      </span>
                    </div>
                  </div>
                    ...
                    {brands.map((brand, idx) => {
                      ...
                      return (
                        <button
                          ...
                          onFocus={pauseMarquee}
                          onBlur={() => resumeMarqueeLater(250)}
                          className="group flex-shrink-0 rounded-2xl bg-white hover:bg-gray-50 border border-gray-100 shadow-sm transition px-3 py-2 sm:px-3.5 sm:py-2.5 hover:cursor-grab active:cursor-grabbing"
                        >
                          <div className="flex flex-col items-center justify-center gap-1 min-w-[116px] sm:min-w-[132px] md:min-w-[140px]">
                            <div className="h-9 w-9 md:h-10 md:w-10 rounded-xl bg-orange-50 flex items-center justify-center text-orange-600">
                              <Icon className="h-5 w-5" />
                            </div>
                            <div className="flex items-center gap-2">
                              <span className="uppercase font-extrabold text-gray-800 text-[11px] lg:text-[13px] truncate max-w-[120px]">
          ...
          <section className="mt-8 sm:mt-10">
            <div className="flex items-end justify-between gap-2 mb-4">
              <div>
                ...
                <p className="hidden sm:block text-gray-600 text-[12px] sm:text-[12px] md:text-[13px] lg:text-[14px] mt-1">
                  Latest products only, sorted by created_date / createdAt.
                </p>
              </div>
            </div>
              ...
              {featuredProducts.map((item, index) => {
                ...
                return (
                  <div
                    ...
                    className="rounded-2xl border border-gray-100 bg-white shadow-sm hover:shadow-md transition-transform duration-300 hover:-translate-y-1 cursor-pointer overflow-hidden"
                    onClick={() => navigate(`/single-product/${productId}`)}
                    title={item.product_name}
                  >
                    <HiddenProductDetails
                    ...
                    />
                    <div className="relative">
                      <div className="w-full aspect-[4/3] bg-white flex items-center justify-center p-2.5">
                        <img
                          src={resolveProductImage(item)}
                          alt={item.product_name}
                    ...
                    <div className="p-2.5">
                      <div className="flex items-center justify-between gap-2 mb-2">
                        <div className="min-w-0 flex gap-1.5 overflow-hidden">
                          <span className="inline-flex items-center rounded-full bg-green-50 text-green-700 border border-green-100 px-2 py-1 text-[9.5px] sm:text-[10px] font-bold">
                            {getAvailabilityLabel(item)}
                          </span>
                          <span className="hidden md:inline-flex items-center rounded-full bg-orange-50 text-orange-700 border border-orange-100 px-2 py-1 text-[9.5px] sm:text-[10px] font-bold">
                            {getDeliveryLabel(item)}
                          </span>
                        </div>
                        ...
                        <button
                          ...
                          onClick={(e) => handleAddToCart(e, item)}
                          disabled={isAdding}
                          className="shrink-0 inline-flex items-center gap-1 rounded-xl bg-orange-500 hover:bg-orange-600 disabled:bg-orange-300 text-white px-2.5 py-1.5 text-[10px] font-extrabold transition shadow-sm"
                        >
                          <FaCartPlus className="h-3 w-3" />
                          {isAdding ? "Adding..." : "Add"}
                        </button>
                      ...
                      <h4 className="text-[12px] font-extrabold text-gray-900 leading-snug line-clamp-2 min-h-[2.15rem]">
                        {item.product_name}
                      </h4>

                      <div className="mt-1 text-[10px] text-gray-500 truncate font-semibold">
                        {String(item?.brand || "No Brand").toUpperCase()}
                      </div>
                      ...
                      <div className="mt-1.5 flex items-end justify-between gap-2">
                        ...
                        {getDisplayPrice(item) > 0 && (
                          <p className="text-gray-400 line-through text-[10px] truncate">
                            {formatCurrency(getDisplayPrice(item))}
                          </p>
                        )}
                      </div>
        ...
        {brandedProducts.length > 0 && (
          <section className="mt-10 sm:mt-14 mb-10 sm:mb-16">
            <div className="flex flex-col sm:flex-row sm:items-end sm:justify-between gap-3 mb-4">
              <div>
                <h2 className="text-[18px] sm:text-[22px] md:text-[26px] lg:text-[28px] font-extrabold text-gray-900 tracking-tight">
                  Explore Products from Popular Brands
                ...
                <p className="hidden sm:block text-gray-600 text-[12px] sm:text-[12px] md:text-[13px] lg:text-[14px] mt-1">
                  More products without repeating the latest featured picks.
                </p>
              </div>
              ...
              <div className="grid grid-cols-2 sm:grid-cols-4 md:grid-cols-5 lg:grid-cols-7 xl:grid-cols-7 gap-3 sm:gap-4">
                {Array.from({ length: 12 }).map((_, index) => (
                  ...
                  >
                    ...
                    <div className="p-3">
                      <div className="h-4 rounded bg-gray-100 mb-2" />
                      <div className="h-3 rounded bg-gray-100 mb-2 w-2/3" />
                  ...
                  {brandVisibleProducts.map((item, index) => {
                    ...
                    return (
                      ...
                      >
                        ...
                        />
                        <div className="relative">
                          <div className="w-full aspect-[4/3] bg-white flex items-center justify-center p-2.5 sm:p-3">
                            <img
                              src={resolveProductImage(item)}
                              alt={item.product_name}
                              className="w-full h-full object-cover"
                ...
                <div className="flex justify-center mt-6">
                  {hasMoreBrandProducts ? (
                    <button
                      type="button"
                      onClick={() =>
                        setVisibleProductCount((prev) => prev + 32)
                      }
                      className="rounded-full border border-orange-200 bg-white px-5 py-2.5 text-[12px] sm:text-[13px] font-extrabold text-orange-600 shadow-sm hover:bg-orange-50 transition"
                    >
                      Load more products
                    </button>
                  ) : (
