...
// - Brand marquee: left/right indicators now slide smoothly (visible slow transition)
// - Brand marquee: pause on hover, resume on leave (kept)
// - Brand products: show more + show less (kept)
// - NOTHING else removed/changed
...
//   /** ✅ Smooth apply for indicator-click (slow slide) */
//   const applyManualOffsetSmooth = useCallback(
//     (durationMs = 900) => {
//       const track = marqueeTrackRef.current;
//       if (!track) return;

//       // Enable transition for this one move
//       track.style.transition = `transform ${durationMs}ms ease`;
//       applyManualOffset();

//       // Remove transition after it finishes so marquee animation stays normal
//       window.setTimeout(() => {
...
//   // ✅ Smooth "Next section" slide for indicator clicks
...
//     // ✅ smooth visible slide
//     applyManualOffsetSmooth(950);

//     // resume a bit later (after the slide completes)
//     resumeMarqueeLater(1050);
//   }, [pauseMarquee, applyManualOffsetSmooth, resumeMarqueeLater]);
...
//   // ============================
//   // ✅ Brand products "Show more/Show less" logic (NO UI redesign)
//   // ============================
//   const brandCols = useMemo(() => Math.max(2, brandGridCols), [brandGridCols]);

//   // Mobile: 4 rows initially, +4 rows per click
//   // Desktop: 2 rows initially, +2 rows per click
...
//   const hasLessBrandProducts =
//     brandVisibleCount > Math.min(brandedProducts.length, initialCount);
...
//             {(hasMoreBrandProducts || hasLessBrandProducts) && (
...
// ✅ file: src/pages/home_pages/Homepage.jsx
// ✅ ONLY LOGIC CHANGE (UI unchanged):
// - Brand marquee: left/right indicators slide smoothly (visible slow transition)
// - Brand marquee: pause on hover, resume on leave (kept)
// - Brand marquee: ✅ DRAG works even over brand buttons (fixed: temporarily disable CSS animation)
// - Brand marquee: ✅ grab cursor only while hovering brand cards (instant, no delay)
// - Brand products: show more + show less (kept)
// - NOTHING else removed/changed
...
const Homepage = () => {
  ...
  // ✅ when we manually control transform, we temporarily disable CSS animation
  const manualAnimationOffRef = useRef(false);
  ...
  const turnOffTrackAnimation = useCallback(() => {
    const track = marqueeTrackRef.current;
    if (!track) return;
    manualAnimationOffRef.current = true;
    track.style.animation = "none"; // ✅ critical
  }, []);

  const turnOnTrackAnimation = useCallback(() => {
    const track = marqueeTrackRef.current;
    if (!track) return;
    manualAnimationOffRef.current = false;
    track.style.animation = ""; // restore to CSS class animation
  }, []);
  ...
  const applyManualOffset = useCallback(() => {
    ...
    // ✅ works only because we disable animation while dragging / button-slide
    track.style.transform = `translate3d(${-o}px, 0, 0)`;
  }, []);
  ...
  const onDown = (clientX) => {
    ...
    // ✅ IMPORTANT: disable CSS animation so manual transform takes effect
  ...
  const onUp = () => {
    ...
    // ✅ restore animation before resuming marquee
  ...
  const jumpToNextBrandSet = useCallback(() => {
    ...
    // ✅ disable animation so transform slide is visible
    turnOffTrackAnimation();
    ...
    // ✅ restore + resume after slide finishes
    window.setTimeout(() => {
      turnOnTrackAnimation();
                      ...
                      return (
                        <button
                          ...
                          className={[
                            ...
                            "transition",
                            "px-3 py-2 sm:px-3.5 sm:py-2.5",
                            // ✅ cursor grab ONLY on brand cards (instant)
...
// // second code removed show more and show less buttons from products from popular brands section .

// // ✅ file: src/pages/home_pages/Homepage.jsx
// // ✅ ONLY LOGIC CHANGE (UI unchanged) + requested update:
// // - Brand marquee: smooth indicator slide + pause/resume + drag works over buttons (kept)
// // - Brand products grid: ✅ NO Show more/Show less buttons
// // - Brand products grid: ✅ Auto “load more on scroll down” + “hide back on scroll up”
// // - Very fast: uses IntersectionObserver (no heavy scroll calculations)
...
// //   // ============================================================
// //   // ✅ BRAND PRODUCTS AUTO LOAD/HIDE (VERY FAST)
// //   // - uses IntersectionObserver (no heavy scroll math)
// //   // - bottom sentinel: when visible AND scrolling down => show next rows
// //   // - top sentinel: when visible AND scrolling up   => hide back rows
// //   // ============================================================
...
// //             setBrandVisibleCount((prev) => clampCount(prev + stepCount));
// //           }
...
// //       {
// //         root: null,
// //         // ✅ triggers slightly before reaching the edge => feels smooth
...
// //   }, [brandedProducts.length, stepCount, clampCount]);
...
// //         /* ✅ very light appear animation for new cards (fast) */
...
// //         {/* === BRAND PRODUCTS GRID (AUTO SCROLL LOAD/HIDE) === */}
// //         {brandedProducts.length > 0 && (
// //           <section
// //             ref={brandSectionRef}
// //             style={{
...
// //                 {/* ✅ Hide in MOBILE */}
// //                 <p className="hidden sm:block text-gray-600 text-[12px] sm:text-[12px] md:text-[13px] lg:text-[14px] mt-1">
// //                   Scroll to auto-load more products (scroll up to hide).
// //                 </p>
// //               </div>
...
// //             {/* ✅ top sentinel (for hiding when scrolling up) */}
...
// //                   ].join(" ")}
// //                   style={{
// //                     animation: "fadeInUpFast 160ms ease-out",
// //                   }}
// //                   onClick={() => navigate(`/single-product/${item._id}`)}
// //                   title={item.product_name}
// //                 >
...
// //             {/* ✅ bottom sentinel (for loading more when scrolling down) */}
