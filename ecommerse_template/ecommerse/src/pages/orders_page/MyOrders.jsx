...
// till here original code. for testing .

// ✅ PREMIUM AMAZON-LEVEL ORDERS PAGE (FINAL)
...
${order.items
  .map(
    (i) =>
      `${i.product_name} x ${i.quantity} = ₹${i.selling_price * i.quantity}`,
  )
  .join("\n")}
    `;
  ...
  return (
    <div className="w-full px-3 sm:px-6 lg:px-10 py-6">
      {/* HEADER */}
      <h1 className="text-[22px] sm:text-[28px] font-extrabold mb-6">
        My Orders
      </h1>

      {orders.length === 0 ? (
        <div className="text-center mt-20">
          <FaClock className="text-3xl mx-auto text-gray-400" />
          <p className="mt-3 font-bold text-sm">No orders yet</p>
        </div>
      ) : (
        <div className="space-y-6">
          {orders.map((order) => {
            const open = openOrder === order._id;
            ...
            return (
              <div key={order._id} className="bg-white rounded-xl border p-4">
                ...
                <div
                  ...
                  onClick={() => setOpenOrder(open ? null : order._id)}
                >
                  <div>
                    <p className="text-[12px] font-bold break-all">
                      {order._id}
                    </p>
                  ...
                  <div className="mt-4 space-y-4">
                    ...
                    <div className="flex flex-wrap gap-2 justify-end">
                      <button
                        ...
                        className="text-[11px] px-3 py-1.5 bg-slate-800 text-white rounded-md flex items-center gap-1"
                      >
                        <FaRedo /> Reorder
                      </button>
                      ...
                      <button
                        ...
                        className="text-[11px] px-3 py-1.5 bg-orange-500 text-white rounded-md flex items-center gap-1"
                      >
                        <FaFileInvoice /> Invoice
                      </button>

                      <button className="text-[11px] px-3 py-1.5 bg-gray-200 rounded-md flex items-center gap-1">
