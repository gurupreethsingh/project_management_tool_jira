...
export default function SearchProducts() {
  ...
  useEffect(() => {
    ...
    debounceRef.current = setTimeout(() => {
      ...
      const matched = allProducts.filter((product) => {
        ...
        const sellingPrice = String(product?.selling_price ?? "").toLowerCase();
        const displayPrice = String(product?.display_price ?? "").toLowerCase();
        const actualPrice = String(product?.actual_price ?? "").toLowerCase();
        const normalPrice = String(product?.price ?? "").toLowerCase();
        ...
        return (
          ...
          sellingPrice.includes(lowerQuery) ||
          displayPrice.includes(lowerQuery) ||
          actualPrice.includes(lowerQuery) ||
          normalPrice.includes(lowerQuery) ||
  ...
  // 1. Clears query state
  // 2. Clears URL ?query=samsung
  // 3. Shows all products
  // 4. Removes "Results for samsung"
  // 5. Refresh will not bring back searched keyword
  const handleClearAllFilters = useCallback(() => {
    ...
    navigate("/search-products", { replace: true });
  ...
  return (
    <div className="shop-font w-full shop-scope">
      <style>{`
        ...
        .brandScrollArea::-webkit-scrollbar {
          width: 6px;
        }

        .brandScrollArea::-webkit-scrollbar-track {
          background: rgba(241,245,249,1);
          border-radius: 9999px;
        }

        .brandScrollArea::-webkit-scrollbar-thumb {
        ...
        @media (max-width: 767px){
          .shopGridResponsive{ grid-template-columns: repeat(2, minmax(0, 1fr)) !important; column-gap: 14px !important; row-gap: 24px !important; }
          .shopGridImageWrap{ padding: 0 !important; background: transparent !important; }
          .shopGridImageBox{ border-radius: 14px !important; }
          .shopGridDetails{ margin-top: 9px !important; padding: 0 !important; background: transparent !important; border: 0 !important; }
          .shopGridTitle{ font-size: 11.5px !important; line-height: 1.05rem !important; white-space: normal !important; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; min-height: 2.1rem; }
          .shopGridMeta{ margin-top: 4px !important; font-size: 9.5px !important; line-height: .9rem !important; white-space: nowrap !important; overflow: hidden; text-overflow: ellipsis; }
          ...
          .shopGridPriceRow{ margin-top: 6px !important; gap: 5px !important; flex-wrap: wrap; align-items: center !important; }
          ...
          .shopGridCartBtn{ margin-top: 8px !important; min-height: 34px !important; padding: 8px 6px !important; font-size: 9.8px !important; line-height: 1 !important; gap: 5px !important; white-space: nowrap !important; }
        ...
        @media (min-width: 768px) and (max-width: 1023px){
          ...
          .shopGridTitle{ font-size: 12.5px !important; line-height: 1.15rem !important; white-space: normal !important; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden; min-height: 2.3rem; }
            ...
            >
              <div className="h-full flex flex-col">
                ...
                <div className="flex-1 overflow-y-auto px-4 py-4">
                  <FiltersSidebar
                    key={`mobile-filters-${filterResetKey}-${query}`}
                    allProducts={baseProducts}
                    filterOptionsProducts={allProducts}
                ...
                {query && (
                  <div className="mt-3 flex flex-wrap items-center gap-2">
                    <button
                      ...
                      className="rounded-full bg-gradient-to-r from-orange-500 to-amber-400 px-4 py-2 text-[12px] font-extrabold text-white shadow-lg shadow-orange-500/20 hover:opacity-95 active:scale-[0.99] transition"
                    >
                      Show All Products
                    </button>
                    ...
                    <button
                      ...
                      className="rounded-full border border-slate-200 bg-white px-4 py-2 text-[12px] font-extrabold text-slate-700 hover:bg-slate-50 transition"
                    >
                      Back to Shop
                    </button>
                  </div>
            ...
            <div className="mt-6 flex flex-col lg:flex-row gap-6 md:gap-10 xl:gap-14">
              <aside className="hidden lg:block w-full lg:w-[320px] xl:w-[340px] sidebarWrap">
                <div className="lg:sticky lg:top-20">
                  <FiltersSidebar
                    key={`desktop-filters-${filterResetKey}-${query}`}
                    allProducts={baseProducts}
                    filterOptionsProducts={allProducts}
                    onFilterChange={handleFilterChange}
                    onClearAllFilters={handleClearAllFilters}
...
function FiltersSidebar({
  ...
  const optionProducts = useMemo(() => {
    const source =
      Array.isArray(filterOptionsProducts) && filterOptionsProducts.length > 0
        ? filterOptionsProducts
        : allProducts;

    return (source || []).filter((p) => p?.isDeleted !== true);
  }, [filterOptionsProducts, allProducts]);
  ...
  useEffect(() => {
    if (optionProducts.length > 0) {
      ...
      setBrands(
        [...brandSet].sort((a, b) => String(a).localeCompare(String(b))),
      );

      const prices = optionProducts
  ...
  return (
    ...
    >
      <div className="flex justify-center">
        ...
        >
          {initialQuery ? "Show All Products" : "Clear Filters"}
        </motion.button>
      </div>
                    ...
                    <div
                      ...
                      onClick={() => {
                        setSelectedBrands((prev) => {
                          const alreadySelected = prev.includes(brand);
...
function ProductsGridUI({
  ...
  return (
    <div className="shopProductsGrid shopGridResponsive grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 xl:grid-cols-6 gap-x-4 sm:gap-x-6 lg:gap-x-8 gap-y-8 sm:gap-y-10 lg:gap-y-12">
      {products.map((p, idx) => {
        const id = p?._id ?? `${idx}`;
