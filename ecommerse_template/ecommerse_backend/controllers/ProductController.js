// const Product = require("../models/ProductModel");
// const multer = require("multer");
// const fs = require("fs");
// const path = require("path");

// // ======================
// // 🗂️ Multer Configuration
// // ======================
// const productUploadDir = path.join("uploads", "products");

// if (!fs.existsSync(productUploadDir)) {
//   fs.mkdirSync(productUploadDir, { recursive: true });
// }

// const productStorage = multer.diskStorage({
//   destination: (req, file, cb) => {
//     cb(null, productUploadDir);
//   },
//   filename: (req, file, cb) => {
//     const filename = `${file.fieldname}-${Date.now()}${path.extname(
//       file.originalname
//     )}`;
//     cb(null, filename);
//   },
// });

// const productUpload = multer({ storage: productStorage });
// exports.productUpload = productUpload;

// // ======================
// // 🔧 Helpers
// // ======================
// const normalizePath = (p) =>
//   String(p || "")
//     .replace(/\\/g, "/")
//     .trim();

// const uniq = (arr) => {
//   const seen = new Set();
//   const out = [];
//   for (const v of arr) {
//     const n = normalizePath(v);
//     if (!n) continue;
//     if (!seen.has(n)) {
//       seen.add(n);
//       out.push(n);
//     }
//   }
//   return out;
// };

// const MAX_GALLERY_IMAGES = 5;

// // ======================
// // 🆕 Create Product
// // ======================
// exports.createProduct = async (req, res) => {
//   try {
//     // Validation: Vendor and Outlet are required
//     if (!req.body.vendor || !req.body.outlet) {
//       return res
//         .status(400)
//         .json({ message: "Vendor and Outlet are required." });
//     }

//     // Remove empty optional fields
//     if (req.body.subcategory === "") delete req.body.subcategory;
//     if (req.body.category === "") delete req.body.category;

//     const {
//       product_name,
//       slug,
//       description,
//       sku,
//       selling_price,
//       display_price,
//       category,
//       subcategory,
//       brand,
//       barcode,
//       stock,
//       warehouse_stock,
//       total_products_sold,
//       outlet,
//       dimensions,
//       color,
//       material,
//       ratings,
//       avg_rating,
//       total_reviews,
//       tags,
//       section_to_appear,
//       featured,
//       is_new_arrival,
//       is_trending,
//       availability_status,
//       discount,
//       min_purchase_qty,
//       max_purchase_qty,
//       delivery_time_estimate,
//       replacement_policy,
//       origin_country,
//       pricing_rules,
//       campaign,
//       vendor,
//       reviews,
//       orders,
//       purchases,
//       returns,
//       wishlist_users,
//       questions,
//       related_products,
//       bundles,
//       vector_embedding,
//       popularity_score,
//       meta_title,
//       meta_description,
//       createdBy,
//       updatedBy,
//       version,
//       admin_notes,
//     } = req.body;

//     // 🛡️ Safe optional chaining for images
//     const mainImage = req.files?.["product_image"]?.[0]?.path
//       ? normalizePath(req.files["product_image"][0].path)
//       : "";

//     const galleryImagesRaw = req.files?.["all_product_images"]
//       ? req.files["all_product_images"].map((f) => normalizePath(f.path))
//       : [];

//     const galleryImages = uniq(galleryImagesRaw);

//     // ✅ Enforce max 5 gallery images
//     if (galleryImages.length > MAX_GALLERY_IMAGES) {
//       return res.status(400).json({
//         message: `Only ${MAX_GALLERY_IMAGES} gallery images are allowed.`,
//       });
//     }

//     const newProduct = new Product({
//       product_name,
//       slug,
//       description,
//       sku,
//       selling_price,
//       display_price,
//       product_image: mainImage,
//       all_product_images: galleryImages,
//       category,
//       subcategory,
//       brand,
//       barcode,
//       stock,
//       warehouse_stock,
//       total_products_sold,
//       outlet,
//       dimensions,
//       color,
//       material,
//       ratings,
//       avg_rating,
//       total_reviews,
//       tags,
//       section_to_appear,
//       featured,
//       is_new_arrival,
//       is_trending,
//       availability_status,
//       discount,
//       min_purchase_qty,
//       max_purchase_qty,
//       delivery_time_estimate,
//       replacement_policy,
//       origin_country,
//       pricing_rules,
//       campaign,
//       vendor,
//       reviews,
//       orders,
//       purchases,
//       returns,
//       wishlist_users,
//       questions,
//       related_products,
//       bundles,
//       vector_embedding,
//       popularity_score,
//       meta_title,
//       meta_description,
//       createdBy,
//       updatedBy,
//       version,
//       admin_notes,
//     });

//     const saved = await newProduct.save();
//     res.status(201).json(saved);
//   } catch (error) {
//     console.error("Create Product Error:", error);
//     if (error.code === 11000 && error.keyPattern?.sku) {
//       return res
//         .status(400)
//         .json({ message: "SKU already exists. Please use a unique SKU." });
//     }
//     res.status(500).json({ message: "Failed to create product." });
//   }
// };

// // ======================
// // 📦 Get All Products
// // ======================
// exports.getAllProducts = async (req, res) => {
//   try {
//     const products = await Product.find({ isDeleted: false })
//       .populate("category")
//       .populate("subcategory")
//       .populate("vendor");
//     res.status(200).json(products);
//   } catch (error) {
//     console.error("Get All Products Error:", error);
//     res.status(500).json({ message: "Failed to fetch products." });
//   }
// };

// // ======================
// // 🔍 Get Product By ID
// // ======================
// exports.getProductById = async (req, res) => {
//   try {
//     const product = await Product.findById(req.params.id)
//       .populate("category")
//       .populate("subcategory")
//       .populate("vendor")
//       .populate({
//         path: "related_products",
//         select: "product_name product_image selling_price",
//       })
//       .populate({
//         path: "bundles.items.product",
//         select: "product_name product_image selling_price",
//       });

//     if (!product || product.isDeleted) {
//       return res.status(404).json({ message: "Product not found." });
//     }

//     // ✅ Always return de-duped gallery list
//     product.all_product_images = uniq(product.all_product_images || []);

//     res.status(200).json(product);
//   } catch (error) {
//     console.error("Get Product By ID Error:", error);
//     res.status(500).json({ message: "Failed to fetch product." });
//   }
// };

// // ======================
// // ➕ Add ONE Gallery Image (one-by-one, max 5)
// // ======================
// exports.addOneGalleryImage = async (req, res) => {
//   try {
//     const productId = req.params.id;

//     if (!req.file?.path) {
//       return res.status(400).json({ message: "Gallery image is required." });
//     }

//     const imgPath = normalizePath(req.file.path);

//     const product = await Product.findById(productId);
//     if (!product || product.isDeleted) {
//       return res.status(404).json({ message: "Product not found." });
//     }

//     const current = uniq(product.all_product_images || []);

//     if (current.length >= MAX_GALLERY_IMAGES) {
//       return res.status(400).json({
//         message: `Maximum ${MAX_GALLERY_IMAGES} gallery images allowed.`,
//       });
//     }

//     // ✅ prevent duplicates
//     if (current.includes(imgPath)) {
//       return res.status(200).json({
//         message: "Image already exists in gallery.",
//         product: product,
//       });
//     }

//     current.push(imgPath);

//     product.all_product_images = current;
//     product.updatedAt = Date.now();

//     const saved = await product.save();
//     res.status(200).json({
//       message: "Gallery image added.",
//       product: saved,
//     });
//   } catch (error) {
//     console.error("Add Gallery Image Error:", error);
//     res.status(500).json({ message: "Failed to add gallery image." });
//   }
// };

// // ======================
// // ❌ Remove ONE Gallery Image (optional but useful)
// // ======================
// exports.removeOneGalleryImage = async (req, res) => {
//   try {
//     const productId = req.params.id;
//     const { imagePath } = req.body;

//     if (!imagePath) {
//       return res.status(400).json({ message: "imagePath is required." });
//     }

//     const normalized = normalizePath(imagePath);

//     const product = await Product.findById(productId);
//     if (!product || product.isDeleted) {
//       return res.status(404).json({ message: "Product not found." });
//     }

//     const current = uniq(product.all_product_images || []);
//     const filtered = current.filter((p) => normalizePath(p) !== normalized);

//     product.all_product_images = filtered;
//     product.updatedAt = Date.now();

//     const saved = await product.save();
//     res.status(200).json({
//       message: "Gallery image removed.",
//       product: saved,
//     });
//   } catch (error) {
//     console.error("Remove Gallery Image Error:", error);
//     res.status(500).json({ message: "Failed to remove gallery image." });
//   }
// };

// // ======================
// // ✏️ Update Product
// // ======================
// exports.updateProductById = async (req, res) => {
//   try {
//     const cleanArray = (arr) => {
//       if (typeof arr === "string") {
//         try {
//           const parsed = JSON.parse(arr);
//           return Array.isArray(parsed) ? parsed : [];
//         } catch {
//           return [];
//         }
//       }
//       return Array.isArray(arr) ? arr : [];
//     };

//     const {
//       product_name,
//       slug,
//       description,
//       selling_price,
//       display_price,
//       category,
//       subcategory,
//       brand,
//       barcode,
//       stock,
//       warehouse_stock,
//       total_products_sold,
//       outlet,
//       dimensions,
//       color,
//       material,
//       ratings,
//       avg_rating,
//       total_reviews,
//       tags,
//       section_to_appear,
//       featured,
//       is_new_arrival,
//       is_trending,
//       availability_status,
//       discount,
//       min_purchase_qty,
//       max_purchase_qty,
//       delivery_time_estimate,
//       replacement_policy,
//       origin_country,
//       pricing_rules,
//       campaign,
//       vendor,
//       reviews,
//       orders,
//       purchases,
//       returns,
//       wishlist_users,
//       questions,
//       related_products,
//       bundles,
//       vector_embedding,
//       popularity_score,
//       meta_title,
//       meta_description,
//       updatedBy,
//       version,
//       admin_notes,
//     } = req.body;

//     // 🧠 Safe parsing of pricing_rules
//     let safePricingRules = [];
//     if (pricing_rules) {
//       if (typeof pricing_rules === "string") {
//         try {
//           const parsed = JSON.parse(pricing_rules);
//           if (Array.isArray(parsed)) safePricingRules = parsed;
//         } catch {
//           safePricingRules = [];
//         }
//       } else if (Array.isArray(pricing_rules)) {
//         safePricingRules = pricing_rules;
//       }
//     }

//     const updatedFields = {
//       product_name,
//       slug,
//       description,
//       selling_price: selling_price ? Number(selling_price) : 0,
//       display_price: display_price ? Number(display_price) : 0,
//       category,
//       subcategory,
//       brand,
//       barcode,
//       stock: stock ? Number(stock) : 0,
//       warehouse_stock: cleanArray(warehouse_stock),
//       total_products_sold: total_products_sold
//         ? Number(total_products_sold)
//         : 0,
//       outlet,
//       dimensions,
//       color,
//       material,
//       ratings: ratings ? Number(ratings) : 0,
//       avg_rating: avg_rating ? Number(avg_rating) : 0,
//       total_reviews: total_reviews ? Number(total_reviews) : 0,
//       tags: cleanArray(tags),
//       section_to_appear: cleanArray(section_to_appear),
//       featured: featured === "true" || featured === true,
//       is_new_arrival: is_new_arrival === "true" || is_new_arrival === true,
//       is_trending: is_trending === "true" || is_trending === true,
//       availability_status:
//         availability_status === "true" || availability_status === true,
//       discount: discount ? Number(discount) : 0,
//       min_purchase_qty: min_purchase_qty ? Number(min_purchase_qty) : 1,
//       max_purchase_qty: max_purchase_qty ? Number(max_purchase_qty) : 100,
//       delivery_time_estimate,
//       replacement_policy,
//       origin_country,
//       pricing_rules: safePricingRules,
//       campaign,
//       vendor,
//       reviews: cleanArray(reviews),
//       orders: cleanArray(orders),
//       purchases: cleanArray(purchases),
//       returns: cleanArray(returns),
//       wishlist_users: cleanArray(wishlist_users),
//       questions: cleanArray(questions),
//       related_products: cleanArray(related_products),
//       bundles: cleanArray(bundles),
//       vector_embedding: cleanArray(vector_embedding),
//       popularity_score: popularity_score ? Number(popularity_score) : 0,
//       meta_title,
//       meta_description,
//       updatedBy,
//       version: version ? Number(version) : 1,
//       admin_notes,
//       updatedAt: Date.now(),
//     };

//     // 🖼️ Handle optional main image safely
//     if (req.files?.["product_image"]?.[0]?.path) {
//       updatedFields.product_image = normalizePath(
//         req.files["product_image"][0].path
//       );
//     }

//     // ✅ Gallery: MERGE + DEDUPE + CAP at 5
//     // (so even if client sends multiple, you won't get duplicates)
//     if (req.files?.["all_product_images"]?.length) {
//       const incoming = req.files["all_product_images"].map((f) =>
//         normalizePath(f.path)
//       );

//       const product = await Product.findById(req.params.id);
//       if (!product || product.isDeleted) {
//         return res.status(404).json({ message: "Product not found." });
//       }

//       const merged = uniq([...(product.all_product_images || []), ...incoming]);

//       if (merged.length > MAX_GALLERY_IMAGES) {
//         return res.status(400).json({
//           message: `Only ${MAX_GALLERY_IMAGES} gallery images are allowed.`,
//         });
//       }

//       updatedFields.all_product_images = merged;
//     }

//     const updated = await Product.findByIdAndUpdate(
//       req.params.id,
//       updatedFields,
//       { new: true }
//     );

//     if (!updated)
//       return res.status(404).json({ message: "Product not found." });

//     // ✅ Ensure response is clean
//     updated.all_product_images = uniq(updated.all_product_images || []);

//     res.status(200).json(updated);
//   } catch (error) {
//     console.error("Update Product Error:", error);
//     res.status(500).json({ message: "Failed to update product." });
//   }
// };

// // ======================
// // ❌ Soft Delete Product
// // ======================
// exports.deleteProductById = async (req, res) => {
//   try {
//     const deleted = await Product.findByIdAndUpdate(
//       req.params.id,
//       { isDeleted: true, updatedAt: Date.now() },
//       { new: true }
//     );
//     if (!deleted)
//       return res.status(404).json({ message: "Product not found." });
//     res.status(200).json({ message: "Product deleted." });
//   } catch (error) {
//     console.error("Delete Product Error:", error);
//     res.status(500).json({ message: "Failed to delete product." });
//   }
// };

// // ======================
// // 📊 Counting APIs
// // ======================
// exports.countAllProducts = async (req, res) => {
//   try {
//     const count = await Product.countDocuments({ isDeleted: false });
//     res.status(200).json({ count });
//   } catch {
//     res.status(500).json({ message: "Count failed" });
//   }
// };

// exports.countProductsByCategory = async (req, res) => {
//   try {
//     const data = await Product.aggregate([
//       { $match: { isDeleted: false } },
//       { $group: { _id: "$category", count: { $sum: 1 } } },
//     ]);
//     res.status(200).json(data);
//   } catch {
//     res.status(500).json({ message: "Count by category failed" });
//   }
// };

// exports.countProductsBySubCategory = async (req, res) => {
//   try {
//     const data = await Product.aggregate([
//       { $match: { isDeleted: false } },
//       { $group: { _id: "$subcategory", count: { $sum: 1 } } },
//     ]);
//     res.status(200).json(data);
//   } catch {
//     res.status(500).json({ message: "Count by subcategory failed" });
//   }
// };

// exports.countProductsByVendor = async (req, res) => {
//   try {
//     const data = await Product.aggregate([
//       { $match: { isDeleted: false } },
//       { $group: { _id: "$vendor", count: { $sum: 1 } } },
//     ]);
//     res.status(200).json(data);
//   } catch {
//     res.status(500).json({ message: "Count by vendor failed" });
//   }
// };

// exports.countProductsByStatus = async (req, res) => {
//   try {
//     const available = await Product.countDocuments({
//       availability_status: true,
//       isDeleted: false,
//     });
//     const unavailable = await Product.countDocuments({
//       availability_status: false,
//       isDeleted: false,
//     });
//     res.status(200).json({ available, unavailable });
//   } catch {
//     res.status(500).json({ message: "Count by status failed" });
//   }
// };

// exports.countProductsBySection = async (req, res) => {
//   try {
//     const data = await Product.aggregate([
//       { $match: { isDeleted: false } },
//       { $unwind: "$section_to_appear" },
//       { $group: { _id: "$section_to_appear", count: { $sum: 1 } } },
//     ]);
//     res.status(200).json(data);
//   } catch {
//     res.status(500).json({ message: "Section count failed" });
//   }
// };

// // ======================
// // 🔎 Filtering APIs
// // ======================
// exports.getProductsByCategory = async (req, res) => {
//   try {
//     const products = await Product.find({
//       category: req.params.categoryId,
//       isDeleted: false,
//     })
//       .populate("subcategory")
//       .populate("vendor");
//     res.status(200).json(products);
//   } catch {
//     res.status(500).json({ message: "Failed to fetch by category." });
//   }
// };

// exports.getProductsBySubCategory = async (req, res) => {
//   try {
//     const products = await Product.find({
//       subcategory: req.params.subCategoryId,
//       isDeleted: false,
//     })
//       .populate("category")
//       .populate("vendor");
//     res.status(200).json(products);
//   } catch {
//     res.status(500).json({ message: "Failed to fetch by subcategory." });
//   }
// };

// exports.getProductsSorted = async (req, res) => {
//   try {
//     const { field = "createdAt", order = "desc" } = req.query;
//     const sortOrder = order === "asc" ? 1 : -1;
//     const products = await Product.find({ isDeleted: false }).sort({
//       [field]: sortOrder,
//     });
//     res.status(200).json(products);
//   } catch {
//     res.status(500).json({ message: "Failed to sort products." });
//   }
// };

// // ======================
// // 🔍 Search Products
// // ======================
// exports.searchProducts = async (req, res) => {
//   try {
//     const { query } = req.query;
//     if (!query) {
//       return res.status(400).json({ message: "Search query is required" });
//     }

//     const products = await Product.find({
//       isDeleted: false,
//       $or: [
//         { product_name: { $regex: query, $options: "i" } },
//         { description: { $regex: query, $options: "i" } },
//         { brand: { $regex: query, $options: "i" } },
//         { tags: { $regex: query, $options: "i" } },
//         { color: { $regex: query, $options: "i" } },
//         { material: { $regex: query, $options: "i" } },
//         { meta_title: { $regex: query, $options: "i" } },
//         { meta_description: { $regex: query, $options: "i" } },
//       ],
//     }).populate("category subcategory vendor");

//     res.status(200).json(products);
//   } catch (error) {
//     console.error("Search Products Error:", error.message);
//     res.status(500).json({ message: "Failed to search products" });
//   }
// };

// till here original code

const Product = require("../models/ProductModel");
const UserProductHistory = require("../models/UserProductHistoryModel");
const multer = require("multer");
const fs = require("fs");
const path = require("path");

// ======================
// 🗂️ Multer Configuration
// ======================
const productUploadDir = path.join("uploads", "products");

if (!fs.existsSync(productUploadDir)) {
  fs.mkdirSync(productUploadDir, { recursive: true });
}

const productStorage = multer.diskStorage({
  destination: (req, file, cb) => {
    cb(null, productUploadDir);
  },
  filename: (req, file, cb) => {
    const filename = `${file.fieldname}-${Date.now()}${path.extname(
      file.originalname,
    )}`;
    cb(null, filename);
  },
});

const productUpload = multer({ storage: productStorage });
exports.productUpload = productUpload;

// ======================
// 🔧 Helpers
// ======================
const normalizePath = (p) =>
  String(p || "")
    .replace(/\\/g, "/")
    .trim();

const uniq = (arr) => {
  const seen = new Set();
  const out = [];
  for (const v of arr) {
    const n = normalizePath(v);
    if (!n) continue;
    if (!seen.has(n)) {
      seen.add(n);
      out.push(n);
    }
  }
  return out;
};

const MAX_GALLERY_IMAGES = 5;

const safeObjectIdString = (value) => {
  try {
    return String(value);
  } catch {
    return "";
  }
};

// ======================
// 🆕 Create Product
// ======================
exports.createProduct = async (req, res) => {
  try {
    if (!req.body.vendor || !req.body.outlet) {
      return res
        .status(400)
        .json({ message: "Vendor and Outlet are required." });
    }

    if (req.body.subcategory === "") delete req.body.subcategory;
    if (req.body.category === "") delete req.body.category;

    const {
      product_name,
      slug,
      description,
      sku,
      selling_price,
      display_price,
      category,
      subcategory,
      brand,
      barcode,
      stock,
      warehouse_stock,
      total_products_sold,
      outlet,
      dimensions,
      color,
      material,
      ratings,
      avg_rating,
      total_reviews,
      tags,
      section_to_appear,
      featured,
      is_new_arrival,
      is_trending,
      availability_status,
      discount,
      min_purchase_qty,
      max_purchase_qty,
      delivery_time_estimate,
      replacement_policy,
      origin_country,
      pricing_rules,
      campaign,
      vendor,
      reviews,
      orders,
      purchases,
      returns,
      wishlist_users,
      questions,
      related_products,
      bundles,
      vector_embedding,
      popularity_score,
      meta_title,
      meta_description,
      createdBy,
      updatedBy,
      version,
      admin_notes,
    } = req.body;

    const mainImage = req.files?.["product_image"]?.[0]?.path
      ? normalizePath(req.files["product_image"][0].path)
      : "";

    const galleryImagesRaw = req.files?.["all_product_images"]
      ? req.files["all_product_images"].map((f) => normalizePath(f.path))
      : [];

    const galleryImages = uniq(galleryImagesRaw);

    if (galleryImages.length > MAX_GALLERY_IMAGES) {
      return res.status(400).json({
        message: `Only ${MAX_GALLERY_IMAGES} gallery images are allowed.`,
      });
    }

    const newProduct = new Product({
      product_name,
      slug,
      description,
      sku,
      selling_price,
      display_price,
      product_image: mainImage,
      all_product_images: galleryImages,
      category,
      subcategory,
      brand,
      barcode,
      stock,
      warehouse_stock,
      total_products_sold,
      outlet,
      dimensions,
      color,
      material,
      ratings,
      avg_rating,
      total_reviews,
      tags,
      section_to_appear,
      featured,
      is_new_arrival,
      is_trending,
      availability_status,
      discount,
      min_purchase_qty,
      max_purchase_qty,
      delivery_time_estimate,
      replacement_policy,
      origin_country,
      pricing_rules,
      campaign,
      vendor,
      reviews,
      orders,
      purchases,
      returns,
      wishlist_users,
      questions,
      related_products,
      bundles,
      vector_embedding,
      popularity_score,
      meta_title,
      meta_description,
      createdBy,
      updatedBy,
      version,
      admin_notes,
    });

    const saved = await newProduct.save();
    res.status(201).json(saved);
  } catch (error) {
    console.error("Create Product Error:", error);
    if (error.code === 11000 && error.keyPattern?.sku) {
      return res
        .status(400)
        .json({ message: "SKU already exists. Please use a unique SKU." });
    }
    res.status(500).json({ message: "Failed to create product." });
  }
};

// ======================
// 📦 Get All Products
// ======================
exports.getAllProducts = async (req, res) => {
  try {
    const products = await Product.find({ isDeleted: false })
      .populate("category")
      .populate("subcategory")
      .populate("vendor");
    res.status(200).json(products);
  } catch (error) {
    console.error("Get All Products Error:", error);
    res.status(500).json({ message: "Failed to fetch products." });
  }
};

// ======================
// 🔍 Get Product By ID
// ======================
exports.getProductById = async (req, res) => {
  try {
    const product = await Product.findById(req.params.id)
      .populate("category")
      .populate("subcategory")
      .populate("vendor")
      .populate({
        path: "related_products",
        select: "product_name product_image selling_price",
      })
      .populate({
        path: "bundles.items.product",
        select: "product_name product_image selling_price",
      });

    if (!product || product.isDeleted) {
      return res.status(404).json({ message: "Product not found." });
    }

    product.all_product_images = uniq(product.all_product_images || []);

    res.status(200).json(product);
  } catch (error) {
    console.error("Get Product By ID Error:", error);
    res.status(500).json({ message: "Failed to fetch product." });
  }
};

// ======================
// 🎯 Personalized Featured Picks
// Based on user history categories
// ======================
exports.getRecommendedProductsFromHistory = async (req, res) => {
  try {
    const userId = req.user?.id || req.user?._id;
    const limit = Math.max(1, Math.min(Number(req.query.limit) || 12, 30));

    if (!userId) {
      return res.status(401).json({ message: "Unauthorized user." });
    }

    const history = await UserProductHistory.find({ user: userId })
      .sort({ viewedAt: -1 })
      .limit(20)
      .populate({
        path: "product",
        match: { isDeleted: false },
        select: "_id category brand",
      });

    const validHistory = history.filter((item) => item?.product?._id);

    const viewedProductIds = [
      ...new Set(
        validHistory.map((item) => safeObjectIdString(item.product._id)),
      ),
    ];

    const categoryIds = [
      ...new Set(
        validHistory
          .map((item) => item?.product?.category)
          .filter(Boolean)
          .map((id) => safeObjectIdString(id)),
      ),
    ];

    let recommendedProducts = [];

    if (categoryIds.length > 0) {
      recommendedProducts = await Product.find({
        isDeleted: false,
        availability_status: true,
        category: { $in: categoryIds },
        _id: { $nin: viewedProductIds },
      })
        .populate("category")
        .populate("subcategory")
        .populate("vendor")
        .sort({ popularity_score: -1, createdAt: -1 })
        .limit(limit);
    }

    if (!recommendedProducts.length) {
      recommendedProducts = await Product.find({
        isDeleted: false,
        availability_status: true,
        _id: { $nin: viewedProductIds },
      })
        .populate("category")
        .populate("subcategory")
        .populate("vendor")
        .sort({ featured: -1, popularity_score: -1, createdAt: -1 })
        .limit(limit);
    }

    return res.status(200).json({
      count: recommendedProducts.length,
      products: recommendedProducts,
    });
  } catch (error) {
    console.error("getRecommendedProductsFromHistory error:", error);
    return res.status(500).json({
      message: "Failed to fetch recommended featured products.",
    });
  }
};

// ======================
// 🎯 Personalized Popular Brands Products
// Based on user history brands
// ======================
exports.getPopularProductsFromHistoryBrands = async (req, res) => {
  try {
    const userId = req.user?.id || req.user?._id;
    const limit = Math.max(1, Math.min(Number(req.query.limit) || 12, 30));

    if (!userId) {
      return res.status(401).json({ message: "Unauthorized user." });
    }

    const history = await UserProductHistory.find({ user: userId })
      .sort({ viewedAt: -1 })
      .limit(20)
      .populate({
        path: "product",
        match: { isDeleted: false },
        select: "_id category brand",
      });

    const validHistory = history.filter((item) => item?.product?._id);

    const viewedProductIds = [
      ...new Set(
        validHistory.map((item) => safeObjectIdString(item.product._id)),
      ),
    ];

    const brands = [
      ...new Set(
        validHistory
          .map((item) => String(item?.product?.brand || "").trim())
          .filter(Boolean),
      ),
    ];

    let products = [];

    if (brands.length > 0) {
      products = await Product.find({
        isDeleted: false,
        availability_status: true,
        brand: { $in: brands },
        _id: { $nin: viewedProductIds },
      })
        .populate("category")
        .populate("subcategory")
        .populate("vendor")
        .sort({ popularity_score: -1, createdAt: -1 })
        .limit(limit);
    }

    if (!products.length) {
      products = await Product.find({
        isDeleted: false,
        availability_status: true,
        _id: { $nin: viewedProductIds },
      })
        .populate("category")
        .populate("subcategory")
        .populate("vendor")
        .sort({ is_trending: -1, popularity_score: -1, createdAt: -1 })
        .limit(limit);
    }

    return res.status(200).json({
      count: products.length,
      products,
    });
  } catch (error) {
    console.error("getPopularProductsFromHistoryBrands error:", error);
    return res.status(500).json({
      message: "Failed to fetch brand-based products.",
    });
  }
};

// ======================
// ➕ Add ONE Gallery Image
// ======================
exports.addOneGalleryImage = async (req, res) => {
  try {
    const productId = req.params.id;

    if (!req.file?.path) {
      return res.status(400).json({ message: "Gallery image is required." });
    }

    const imgPath = normalizePath(req.file.path);

    const product = await Product.findById(productId);
    if (!product || product.isDeleted) {
      return res.status(404).json({ message: "Product not found." });
    }

    const current = uniq(product.all_product_images || []);

    if (current.length >= MAX_GALLERY_IMAGES) {
      return res.status(400).json({
        message: `Maximum ${MAX_GALLERY_IMAGES} gallery images allowed.`,
      });
    }

    if (current.includes(imgPath)) {
      return res.status(200).json({
        message: "Image already exists in gallery.",
        product: product,
      });
    }

    current.push(imgPath);

    product.all_product_images = current;
    product.updatedAt = Date.now();

    const saved = await product.save();
    res.status(200).json({
      message: "Gallery image added.",
      product: saved,
    });
  } catch (error) {
    console.error("Add Gallery Image Error:", error);
    res.status(500).json({ message: "Failed to add gallery image." });
  }
};

// ======================
// ❌ Remove ONE Gallery Image
// ======================
exports.removeOneGalleryImage = async (req, res) => {
  try {
    const productId = req.params.id;
    const { imagePath } = req.body;

    if (!imagePath) {
      return res.status(400).json({ message: "imagePath is required." });
    }

    const normalized = normalizePath(imagePath);

    const product = await Product.findById(productId);
    if (!product || product.isDeleted) {
      return res.status(404).json({ message: "Product not found." });
    }

    const current = uniq(product.all_product_images || []);
    const filtered = current.filter((p) => normalizePath(p) !== normalized);

    product.all_product_images = filtered;
    product.updatedAt = Date.now();

    const saved = await product.save();
    res.status(200).json({
      message: "Gallery image removed.",
      product: saved,
    });
  } catch (error) {
    console.error("Remove Gallery Image Error:", error);
    res.status(500).json({ message: "Failed to remove gallery image." });
  }
};

// ======================
// ✏️ Update Product
// ======================
exports.updateProductById = async (req, res) => {
  try {
    const cleanArray = (arr) => {
      if (typeof arr === "string") {
        try {
          const parsed = JSON.parse(arr);
          return Array.isArray(parsed) ? parsed : [];
        } catch {
          return [];
        }
      }
      return Array.isArray(arr) ? arr : [];
    };

    const {
      product_name,
      slug,
      description,
      selling_price,
      display_price,
      category,
      subcategory,
      brand,
      barcode,
      stock,
      warehouse_stock,
      total_products_sold,
      outlet,
      dimensions,
      color,
      material,
      ratings,
      avg_rating,
      total_reviews,
      tags,
      section_to_appear,
      featured,
      is_new_arrival,
      is_trending,
      availability_status,
      discount,
      min_purchase_qty,
      max_purchase_qty,
      delivery_time_estimate,
      replacement_policy,
      origin_country,
      pricing_rules,
      campaign,
      vendor,
      reviews,
      orders,
      purchases,
      returns,
      wishlist_users,
      questions,
      related_products,
      bundles,
      vector_embedding,
      popularity_score,
      meta_title,
      meta_description,
      updatedBy,
      version,
      admin_notes,
    } = req.body;

    let safePricingRules = [];
    if (pricing_rules) {
      if (typeof pricing_rules === "string") {
        try {
          const parsed = JSON.parse(pricing_rules);
          if (Array.isArray(parsed)) safePricingRules = parsed;
        } catch {
          safePricingRules = [];
        }
      } else if (Array.isArray(pricing_rules)) {
        safePricingRules = pricing_rules;
      }
    }

    const updatedFields = {
      product_name,
      slug,
      description,
      selling_price: selling_price ? Number(selling_price) : 0,
      display_price: display_price ? Number(display_price) : 0,
      category,
      subcategory,
      brand,
      barcode,
      stock: stock ? Number(stock) : 0,
      warehouse_stock: cleanArray(warehouse_stock),
      total_products_sold: total_products_sold
        ? Number(total_products_sold)
        : 0,
      outlet,
      dimensions,
      color,
      material,
      ratings: ratings ? Number(ratings) : 0,
      avg_rating: avg_rating ? Number(avg_rating) : 0,
      total_reviews: total_reviews ? Number(total_reviews) : 0,
      tags: cleanArray(tags),
      section_to_appear: cleanArray(section_to_appear),
      featured: featured === "true" || featured === true,
      is_new_arrival: is_new_arrival === "true" || is_new_arrival === true,
      is_trending: is_trending === "true" || is_trending === true,
      availability_status:
        availability_status === "true" || availability_status === true,
      discount: discount ? Number(discount) : 0,
      min_purchase_qty: min_purchase_qty ? Number(min_purchase_qty) : 1,
      max_purchase_qty: max_purchase_qty ? Number(max_purchase_qty) : 100,
      delivery_time_estimate,
      replacement_policy,
      origin_country,
      pricing_rules: safePricingRules,
      campaign,
      vendor,
      reviews: cleanArray(reviews),
      orders: cleanArray(orders),
      purchases: cleanArray(purchases),
      returns: cleanArray(returns),
      wishlist_users: cleanArray(wishlist_users),
      questions: cleanArray(questions),
      related_products: cleanArray(related_products),
      bundles: cleanArray(bundles),
      vector_embedding: cleanArray(vector_embedding),
      popularity_score: popularity_score ? Number(popularity_score) : 0,
      meta_title,
      meta_description,
      updatedBy,
      version: version ? Number(version) : 1,
      admin_notes,
      updatedAt: Date.now(),
    };

    if (req.files?.["product_image"]?.[0]?.path) {
      updatedFields.product_image = normalizePath(
        req.files["product_image"][0].path,
      );
    }

    if (req.files?.["all_product_images"]?.length) {
      const incoming = req.files["all_product_images"].map((f) =>
        normalizePath(f.path),
      );

      const product = await Product.findById(req.params.id);
      if (!product || product.isDeleted) {
        return res.status(404).json({ message: "Product not found." });
      }

      const merged = uniq([...(product.all_product_images || []), ...incoming]);

      if (merged.length > MAX_GALLERY_IMAGES) {
        return res.status(400).json({
          message: `Only ${MAX_GALLERY_IMAGES} gallery images are allowed.`,
        });
      }

      updatedFields.all_product_images = merged;
    }

    const updated = await Product.findByIdAndUpdate(
      req.params.id,
      updatedFields,
      { new: true },
    );

    if (!updated) {
      return res.status(404).json({ message: "Product not found." });
    }

    updated.all_product_images = uniq(updated.all_product_images || []);

    res.status(200).json(updated);
  } catch (error) {
    console.error("Update Product Error:", error);
    res.status(500).json({ message: "Failed to update product." });
  }
};

// ======================
// ❌ Soft Delete Product
// ======================
exports.deleteProductById = async (req, res) => {
  try {
    const deleted = await Product.findByIdAndUpdate(
      req.params.id,
      { isDeleted: true, updatedAt: Date.now() },
      { new: true },
    );
    if (!deleted) {
      return res.status(404).json({ message: "Product not found." });
    }
    res.status(200).json({ message: "Product deleted." });
  } catch (error) {
    console.error("Delete Product Error:", error);
    res.status(500).json({ message: "Failed to delete product." });
  }
};

// ======================
// 📊 Counting APIs
// ======================
exports.countAllProducts = async (req, res) => {
  try {
    const count = await Product.countDocuments({ isDeleted: false });
    res.status(200).json({ count });
  } catch {
    res.status(500).json({ message: "Count failed" });
  }
};

exports.countProductsByCategory = async (req, res) => {
  try {
    const data = await Product.aggregate([
      { $match: { isDeleted: false } },
      { $group: { _id: "$category", count: { $sum: 1 } } },
    ]);
    res.status(200).json(data);
  } catch {
    res.status(500).json({ message: "Count by category failed" });
  }
};

exports.countProductsBySubCategory = async (req, res) => {
  try {
    const data = await Product.aggregate([
      { $match: { isDeleted: false } },
      { $group: { _id: "$subcategory", count: { $sum: 1 } } },
    ]);
    res.status(200).json(data);
  } catch {
    res.status(500).json({ message: "Count by subcategory failed" });
  }
};

exports.countProductsByVendor = async (req, res) => {
  try {
    const data = await Product.aggregate([
      { $match: { isDeleted: false } },
      { $group: { _id: "$vendor", count: { $sum: 1 } } },
    ]);
    res.status(200).json(data);
  } catch {
    res.status(500).json({ message: "Count by vendor failed" });
  }
};

exports.countProductsByStatus = async (req, res) => {
  try {
    const available = await Product.countDocuments({
      availability_status: true,
      isDeleted: false,
    });
    const unavailable = await Product.countDocuments({
      availability_status: false,
      isDeleted: false,
    });
    res.status(200).json({ available, unavailable });
  } catch {
    res.status(500).json({ message: "Count by status failed" });
  }
};

exports.countProductsBySection = async (req, res) => {
  try {
    const data = await Product.aggregate([
      { $match: { isDeleted: false } },
      { $unwind: "$section_to_appear" },
      { $group: { _id: "$section_to_appear", count: { $sum: 1 } } },
    ]);
    res.status(200).json(data);
  } catch {
    res.status(500).json({ message: "Section count failed" });
  }
};

// ======================
// 🔎 Filtering APIs
// ======================
exports.getProductsByCategory = async (req, res) => {
  try {
    const products = await Product.find({
      category: req.params.categoryId,
      isDeleted: false,
    })
      .populate("subcategory")
      .populate("vendor");
    res.status(200).json(products);
  } catch {
    res.status(500).json({ message: "Failed to fetch by category." });
  }
};

exports.getProductsBySubCategory = async (req, res) => {
  try {
    const products = await Product.find({
      subcategory: req.params.subCategoryId,
      isDeleted: false,
    })
      .populate("category")
      .populate("vendor");
    res.status(200).json(products);
  } catch {
    res.status(500).json({ message: "Failed to fetch by subcategory." });
  }
};

exports.getProductsSorted = async (req, res) => {
  try {
    const { field = "createdAt", order = "desc" } = req.query;
    const sortOrder = order === "asc" ? 1 : -1;
    const products = await Product.find({ isDeleted: false }).sort({
      [field]: sortOrder,
    });
    res.status(200).json(products);
  } catch {
    res.status(500).json({ message: "Failed to sort products." });
  }
};

// ======================
// 🔍 Search Products
// ======================
exports.searchProducts = async (req, res) => {
  try {
    const { query } = req.query;
    if (!query) {
      return res.status(400).json({ message: "Search query is required" });
    }

    const products = await Product.find({
      isDeleted: false,
      $or: [
        { product_name: { $regex: query, $options: "i" } },
        { description: { $regex: query, $options: "i" } },
        { brand: { $regex: query, $options: "i" } },
        { tags: { $regex: query, $options: "i" } },
        { color: { $regex: query, $options: "i" } },
        { material: { $regex: query, $options: "i" } },
        { meta_title: { $regex: query, $options: "i" } },
        { meta_description: { $regex: query, $options: "i" } },
      ],
    }).populate("category subcategory vendor");

    res.status(200).json(products);
  } catch (error) {
    console.error("Search Products Error:", error.message);
    res.status(500).json({ message: "Failed to search products" });
  }
};
