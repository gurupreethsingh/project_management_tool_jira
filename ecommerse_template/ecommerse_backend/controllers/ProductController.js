...
exports.createProduct = async (req, res) => {
  try {
    ...
    if (!req.body.vendor || !req.body.outlet) {
      return res.status(400).json({ message: "Vendor and Outlet are required." });
    }
...
exports.updateProductById = async (req, res) => {
  try {
    ...
    const updatedFields = {
      ...
      is_new_arrival: is_new_arrival === "true" || is_new_arrival === true,
      is_trending: is_trending === "true" || is_trending === true,
      availability_status: availability_status === "true" || availability_status === true,
      discount: discount ? Number(discount) : 0,
      min_purchase_qty: min_purchase_qty ? Number(min_purchase_qty) : 1,
      max_purchase_qty: max_purchase_qty ? Number(max_purchase_qty) : 100,
      delivery_time_estimate,
    ...
    // 🖼️ Handle optional images safely
    if (req.files?.["product_image"]?.[0]?.path) {
      updatedFields.product_image = req.files["product_image"][0].path.replace(/\\/g, "/");
    }

    if (req.files?.["all_product_images"]) {
      updatedFields.all_product_images = req.files["all_product_images"].map((file) =>
        file.path.replace(/\\/g, "/")
      );
    }
