...
exports.updateDefectStatus = async (req, res) => {
  ...
  let { status, updated_by, assignedDeveloper } = req.body;
  ...
  const normalizeStatus = (raw) => {
    ...
    const map = {
      ...
      "in-progress": "In-progress", // <== choose ONE canonical
      ...
      "unable-to-fix,": "Unable-To-fix",
      "unable-to-fix ": "Unable-To-fix",
      "unable-to-fixs": "Unable-To-fix", // just being defensive

      "not-an-error": "Not-An-Error",
      "not-an-error.": "Not-An-Error",

      "request-for-enhancement": "Request-For-Enhancement",
      "request-for-enhancements": "Request-For-Enhancement",
    };

    return map[key] || raw; // Fallback: store as-is if not in our map
  };
  ...
  try {
    ...
    const normalizedStatus = normalizeStatus(status || bug.status);
    ...
    // if you want to persist which developer it's assigned to:
    if (assignedDeveloper) {
      bug.assignedDeveloper = assignedDeveloper;
    }
    ...
    const historyEntry = {
      ...
      change_description: `Status changed from ${previousStatus} to ${normalizedStatus}`,
      updated_at: new Date(),
    };
    ...
    res.status(200).json({
      message: "Status updated and history saved",
      status: normalizedStatus,
      previousStatus,
    });
  } catch (error) {
