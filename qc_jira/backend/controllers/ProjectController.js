...
//  */
// const CAN_EDIT_AFTER_APPROVAL = new Set([
//   ...PRIVILEGED_ROLES,
//   "test_engineer",
// ]);
...
//   const byNum = new Map((existingSteps || []).map((s) => [Number(s.step_number) || 0, s]));
...
//     const prevStr = f === "approved_date" && prev ? new Date(prev).toISOString().slice(0, 10) : String(prev);
//     const curStr = f === "approved_date" && cur ? new Date(cur).toISOString().slice(0, 10) : String(cur);
//     if (prevStr !== curStr) {
//       items.push(`Changed **footer.${f.replace(/_/g, " ")}**: "${prevStr}" → "${curStr}"`);
//     }
//   });

//   // Steps summary
//   const prevSteps = Array.isArray(existing?.testing_steps) ? existing.testing_steps : [];
//   const curSteps = Array.isArray(nextDoc?.testing_steps) ? nextDoc.testing_steps : [];

//   if (prevSteps.length !== curSteps.length) {
//     items.push(`Testing steps count changed: ${prevSteps.length} → ${curSteps.length}`);
//   } else {
//     const changed = curSteps.some((s, i) => {
//       const p = prevSteps[i] || {};
//       return (
//         String(p.action_description || "") !== String(s.action_description || "") ||
...
//       const hasFail = testSteps.some((s) => String(s?.status || "").trim().toLowerCase() === "fail");
//       if (hasFail) return "Fail";
//       const hasPending = testSteps.some((s) => String(s?.status || "").trim().toLowerCase() === "pending");
...
//         (t) => t.scenario_id && sc._id && String(t.scenario_id) === String(sc._id)
//       );
...
//         defectReportStatus: status === "Fail" ? (defect ? "Present" : "Missing") : "N/A",
...
//     const normalizedSteps = normalizeSteps(testing_steps, canEditStatus, existing.testing_steps);
