...
async function getUserProjectCount(req, res) {
  ...
  try {
    ...
    if (role === "test_engineer") {
      query = { testEngineers: userId };
    } else if (role === "developer") {
      query = { developers: userId };
    } else if (role === "qa_lead") {
      query = { qaLeads: userId };
    } else if (role === "developer_lead") {
      query = { developerLeads: userId };
    } else if (role === "project_manager") {
      query = { projectManagers: userId };
    } else if (role === "admin") {
      query = { admins: userId };
    } else if (role === "superadmin") {
      query = { superAdmins: userId };
    } else {
      query = {
        $or: [
    ...
    const assignedProjectsCount = await Project.countDocuments(query);
...
async function getUserAssignedProjects(req, res) {
  ...
  try {
    ...
    const assignedProjects = await Project.find({
      ...
      .populate("testEngineers", "name email role")
      .populate("qaLeads", "name email role")
      .populate("developerLeads", "name email role")
      .populate("projectManagers", "name email role")
      .populate("admins", "name email role")
      .populate("superAdmins", "name email role")
...
async function getAllTestCasesForProject(req, res) {
  try {
    ...
    const rows = await TestCase.find(filter)
      .select(
        "_id project_id project_name scenario_id scenario_number test_case_name test_case_number requirement_number build_name_or_number module_name pre_condition test_data post_condition severity test_case_type brief_description test_execution_time test_execution_type testing_steps footer createdAt updatedAt",
      )
      .sort(sortSpec)
      .lean();
