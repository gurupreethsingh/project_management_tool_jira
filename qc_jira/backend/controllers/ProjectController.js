...
// // 🔽 NEW: Excel export dependency
// const ExcelJS = require("exceljs");

// /* ----------------------------- NEW: Export projects to XLSX ----------------------------- */
// async function exportProjectsXlsx(req, res) {
//   try {
//     const { search = "" } = req.query;

//     const q = search
//       ? { project_name: { $regex: String(search), $options: "i" } }
//       : {};
...
//         id: String(p._id || ""),
//         name: p.project_name || "",
//         desc: String(p.description || "").replace(/\r?\n/g, " "),
//         domain: p.domain || "",
//         start: p.startDate
//           ? new Date(p.startDate).toISOString().slice(0, 10)
//           : "",
//         end: p.endDate ? new Date(p.endDate).toISOString().slice(0, 10) : "",
//         deadline: p.deadline
...
//     res.end();
//   } catch (e) {
//     console.error("exportProjectsXlsx error:", e);
//     res.status(500).json({
...
// /**
//  * ✅ Roles that can edit step statuses even AFTER approval.
//  * You asked to include test_engineer explicitly.
//  */
// const CAN_EDIT_AFTER_APPROVAL = new Set([...PRIVILEGED_ROLES, "test_engineer"]);

// function userCanEditStatuses(req, nextApprovedBy) {
//   const role = String(req.user?.role || "").toLowerCase();
//   const approverSet = String(nextApprovedBy || "").trim().length > 0;

//   // If not approved yet → anyone can edit.
//   if (!approverSet) return true;

//   // After approval → only roles in CAN_EDIT_AFTER_APPROVAL can edit.
//   return CAN_EDIT_AFTER_APPROVAL.has(role);
// }
...
//   if (s === "pending" || s === "in progress" || s === "na" || s === "n/a")
//     return "Pending";
//   return "Pending";
// }

// /**
//  * Step-number-aware normalizer:
//  * - Uses provided step_number if available, else i+1
//  * - If canEditStatus=false → preserve existing step status when present
//  * - Otherwise use incoming normalized status
//  */
// function normalizeSteps(steps, canEditStatus, existingSteps = []) {
//   const arr = Array.isArray(steps) ? steps : [];
//   const byNum = new Map(
//     (existingSteps || []).map((s) => [Number(s.step_number) || 0, s]),
//   );

//   return arr.map((s, i) => {
//     const num = Number(s?.step_number) || i + 1;
...
//         (t) =>
//           t.scenario_id && sc._id && String(t.scenario_id) === String(sc._id),
//       );
...
//               b.test_case_number &&
//               tc.test_case_number &&
//               String(b.test_case_number) === String(tc.test_case_number),
//           )
//         : null;
...
// // helper for adding a new test case.
// /* ----------------------------- Test case CRUD ------------------------------ */

// function generateTestCaseNumberFromScenario(scenarioNumber = "") {
//   if (!scenarioNumber || typeof scenarioNumber !== "string") return "";

//   const trimmed = scenarioNumber.trim();
//   if (!trimmed) return "";

//   // Expected format: ET-HC-9948 -> TC-HC-9948
//   if (/^ET-/i.test(trimmed)) {
//     return trimmed.replace(/^ET-/i, "TC-");
//   }
...
//     // duplicate guard — one TestCase per Scenario
...
//     const canEditStatus = userCanEditStatuses(req, approvedBy);

//     const normalizedSteps = normalizeSteps(testing_steps, canEditStatus, []);

//     // Generate test case number from scenario number
//     const test_case_number = generateTestCaseNumberFromScenario(
//       scenario_number || "",
//     );
...
//       test_case_number: test_case_number || undefined,
//       test_case_name,
//       requirement_number,
//       build_name_or_number,
//       module_name,
...
//       req,
//       null,
//       `Created test case${test_case_number ? ` (${test_case_number})` : ""}.`,
//     );
...
//       .select(
//         "_id test_case_name test_case_number scenario_number requirement_number build_name_or_number module_name test_case_type testing_steps test_execution_type severity createdAt",
//       )
//       .sort(sortSpec);
...
//     const approvedBy = String(testCase.footer?.approved_by || "").trim();
//     const canEditStatus = userCanEditStatuses(req, approvedBy);
//     const isLockedByApproval = Boolean(approvedBy) && !canEditStatus;
...
//       },
//       __locks: { canEditStatus, isLockedByApproval },
//       ...testCase,
//     });
//   } catch (err) {
...
//     // ✅ NEW: use the relaxed policy (test_engineer allowed after approval)
//     const canEditStatus = userCanEditStatuses(req, nextApprovedBy);

//     // If cannot edit statuses, check if the request tries to change them.
//     if (!canEditStatus) {
//       const incoming = Array.isArray(testing_steps) ? testing_steps : [];
//       const prevSteps = Array.isArray(existing.testing_steps)
//         ? existing.testing_steps
//         : [];

//       const triesToChange = incoming.some((s, i) => {
//         const prev = prevSteps[i] || {};
//         const prevNorm = normStepStatus(prev.status);
//         const nextNorm = normStepStatus(s?.status);
...
//           message:
//             "Test step statuses are locked after approval. Ask an authorized user to update or clear 'Approved By' to unlock.",
//         });
//       }
//     }
...
// multiple test cases we can write for one Scenario
...
async function getTraceabilityMatrix(req, res) {
  ...
  try {
    ...
    const matrix = scenarios.map((sc) => {
      const matchingTestCases = testCases.filter(
        (t) =>
          t.scenario_id && sc._id && String(t.scenario_id) === String(sc._id),
      );

      const primaryTc = matchingTestCases[0] || null;

      const defect = primaryTc
        ? bugs.find(
            (b) =>
              b.test_case_number &&
              primaryTc.test_case_number &&
              String(b.test_case_number) === String(primaryTc.test_case_number),
          )
        : null;

      const status = primaryTc
        ? getTestCaseStatus(primaryTc.testing_steps)
        : "Pending";
      ...
      return {
        ...
        scenarioText: sc.scenario_text,
        moduleName: sc.module?.name || "Unassigned",
        testCaseNumber: primaryTc ? primaryTc.test_case_number : "Missing",
        testCaseStatus: status,
        defectNumber: defect ? defect.bug_id : "N/A",
        defectReportStatus:
          status === "Fail" ? (defect ? "Present" : "Missing") : "N/A",
        testExecutionType: primaryTc
          ? primaryTc.test_execution_type || "Manual"
          : "Manual",
        totalTestCasesForScenario: matchingTestCases.length,
      };
    });
...
async function generateSequentialTestCaseNumber(scenarioNumber, scenarioId) {
  const baseNumber = buildBaseTestCaseNumberFromScenario(scenarioNumber);

  if (!baseNumber) return "";

  const count = await TestCase.countDocuments({ scenario_id: scenarioId });
  return `${baseNumber}-${count + 1}`;
}
...
async function addTestCase(req, res) {
  try {
    ...
    const safeTestCaseName = String(test_case_name || "").trim();

    // Allow multiple test cases for a scenario,
    // but prevent exact duplicate names inside the same scenario.
    const dup = await TestCase.findOne({
      scenario_id,
      test_case_name: safeTestCaseName,
    });

    if (dup) {
      return res.status(409).json({
        message:
          "A test case with the same name already exists for this scenario.",
        testCaseId: dup._id,
      });
    }
    ...
    // Safe fallback numbering.
    // Model pre-validate should also generate this automatically.
    const generatedTestCaseNumber = await generateSequentialTestCaseNumber(
      scenario_number || "",
      scenario_id,
    );
    ...
    const tc = new TestCase({
      ...
      scenario_id,
      scenario_number: scenario_number || undefined,
      test_case_number: generatedTestCaseNumber || undefined,
      test_case_name: safeTestCaseName,
      requirement_number,
      build_name_or_number,
      module_name,
    ...
    await recordChange(
      ...
      req,
      null,
      `Created test case${tc.test_case_number ? ` (${tc.test_case_number})` : ""}.`,
    );
    ...
    if (error?.code === 11000) {
      return res.status(409).json({
        message:
          "Duplicate test case detected. Use a different test case name for this scenario.",
      });
    }
...
async function updateTestCase(req, res) {
  try {
    ...
    if (err?.code === 11000) {
      return res.status(409).json({
        message:
          "Another test case with the same name already exists for this scenario.",
      });
    }
