...
const normalizeTestLevel = (value = "") => {
...
const normalizeTestTypes = (values = []) => {
  const arr = Array.isArray(values) ? values : values ? [values] : [];

  const normalized = arr
    .map((value) => normalizeTestType(value))
    .filter(Boolean);

  return [...new Set(normalized)];
};

const validateScenarioClassification = ({
  testLevel,
  testTypes,
  requireAtLeastOneType = true,
} = {}) => {
  const normalizedLevel = normalizeTestLevel(testLevel || "System") || "System";
  const normalizedTypes = normalizeTestTypes(testTypes);

  if (!TEST_LEVELS.includes(normalizedLevel)) {
    return {
      ok: false,
      message: `Invalid test level. Allowed values: ${TEST_LEVELS.join(", ")}`,
    };
  }

  if (requireAtLeastOneType && normalizedTypes.length === 0) {
    return {
      ok: false,
  ...
  const invalidType = normalizedTypes.find(
    (type) => !TEST_TYPES.includes(type),
  );
  if (invalidType) {
    return {
      ok: false,
      message: `Invalid test type: ${invalidType}`,
    };
  }

  return {
    ok: true,
    testLevel: normalizedLevel,
    testTypes: normalizedTypes,
  };
};
...
const addScenario = async (req, res) => {
  try {
    ...
    const classification = validateScenarioClassification({
      testLevel,
      testTypes,
      requireAtLeastOneType: true,
    });
    ...
    const scenario = new Scenario({
      ...
      module: primaryModuleId,
      modules: uniqueResolved,
      testLevel: classification.testLevel,
      testTypes: classification.testTypes,
      createdBy,
    });
...
const updateScenario = async (req, res) => {
  try {
    ...
    const classificationRequested =
      typeof testLevel !== "undefined" || typeof testTypes !== "undefined";

    if (classificationRequested) {
      const classification = validateScenarioClassification({
        testLevel:
          typeof testLevel !== "undefined" ? testLevel : scenario.testLevel,
        testTypes:
          typeof testTypes !== "undefined" ? testTypes : scenario.testTypes,
        requireAtLeastOneType: true,
      });

      if (!classification.ok) {
        return res.status(400).json({ message: classification.message });
      }

      scenario.testLevel = classification.testLevel;
      scenario.testTypes = classification.testTypes;
    }
...
const getScenariosSimple = async (req, res) => {
  try {
    ...
    const hydrated = hydrateMany(scenarios).map((s) => ({
      ...
      scenario_number: s.scenario_number,
      scenario_text: s.scenario_text,
      testLevel: s.testLevel || "System",
      testTypes: Array.isArray(s.testTypes) ? s.testTypes : [],
      module: s.module
        ? { _id: s.module._id, name: s.module.name }
        : s.modules?.[0] || null,
      modules: (s.modules || []).map((m) => ({
...
const searchScenarios = async (req, res) => {
  try {
    ...
    const rows = await Scenario.find(
      ...
      { scenario_text: 1, modules: 1, module: 1, testLevel: 1, testTypes: 1 },
    )
      .limit(MAX_SEARCH_RESULTS)
      .populate("module", "name")
      .populate("modules", "name")
