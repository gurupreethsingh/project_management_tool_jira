...
// updated authmiddleware for login process and expired token and logged out.
...
const protect = async (req, res, next) => {
  ...
  try {
    ...
    return res
      .status(401)
      .json({ code: "TOKEN_INVALID", message: "Token verification failed" });
  }
};

// Role guards remain same...
const isAdmin = (req, res, next) =>
  req.user?.role === "admin"
    ? next()
    : res.status(403).json({ message: "Admin access denied" });
