...
const AllCreatedJobs = () => {
  ...
  const fetchJobs = async (showTableLoader = false) => {
    try {
      ...
      const response = await axios.get(
        `${globalBackendRoute}/api/jobs?${queryString}`,
        {
          headers: authHeaders,
        },
      );
      ...
      setServerError(error?.response?.data?.message || "Failed to fetch jobs.");
    } finally {
      setLoading(false);
  ...
  const fetchCounts = async () => {
    try {
      const response = await axios.get(
        `${globalBackendRoute}/api/jobs-counts-summary`,
        {
          headers: authHeaders,
        },
      );
  ...
  const fetchFilterOptions = async () => {
    try {
      const response = await axios.get(
        `${globalBackendRoute}/api/jobs-filter-options`,
        {
          headers: authHeaders,
        },
      );
  ...
  const doSingleSoftDelete = async (jobId) => {
    const confirmed = window.confirm(
      "Are you sure you want to soft delete this job?",
    );
    if (!confirmed) return;

    try {
      const response = await axios.delete(
        `${globalBackendRoute}/api/jobs/${jobId}/soft-delete`,
        {
          headers: authHeaders,
        },
      );
      setServerMessage(response?.data?.message || "Job deleted.");
      fetchJobs(true);
      fetchCounts();
      setSelectedJobIds((prev) => prev.filter((id) => id !== jobId));
    } catch (error) {
      console.error("Single soft delete error:", error);
      setServerError(error?.response?.data?.message || "Failed to delete job.");
    }
  };

  const doSinglePublishToggle = async (jobId) => {
    try {
      const response = await axios.patch(
        `${globalBackendRoute}/api/jobs/${jobId}/toggle-publish`,
        {},
        { headers: authHeaders },
      );
      setServerMessage(
        response?.data?.message || "Job publish status updated.",
      );
      fetchJobs(true);
      fetchCounts();
    } catch (error) {
  ...
  const doBulkAction = async (type) => {
    if (!selectedJobIds.length) {
      setServerError("Please select at least one job.");
      return;
    }
    ...
    try {
      ...
      if (type === "soft-delete") {
        response = await axios.patch(
          `${globalBackendRoute}/api/jobs/bulk/soft-delete`,
          { jobIds: selectedJobIds },
          { headers: authHeaders },
        );
      } else if (type === "restore") {
        response = await axios.patch(
          `${globalBackendRoute}/api/jobs/bulk/restore`,
          { jobIds: selectedJobIds },
          { headers: authHeaders },
        );
      } else if (type === "publish") {
        response = await axios.patch(
          `${globalBackendRoute}/api/jobs/bulk/publish`,
          { jobIds: selectedJobIds },
          { headers: authHeaders },
        );
      } else if (type === "unpublish") {
        response = await axios.patch(
          `${globalBackendRoute}/api/jobs/bulk/unpublish`,
          { jobIds: selectedJobIds },
          { headers: authHeaders },
        );
      } else if (type === "hard-delete") {
        const confirmed = window.confirm(
          "This will permanently delete the selected jobs. Continue?",
        );
        if (!confirmed) return;

        response = await axios.delete(
          `${globalBackendRoute}/api/jobs/bulk/hard-delete`,
          {
            headers: authHeaders,
        ...
        <div className="mb-4 flex flex-col gap-3 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm lg:flex-row lg:items-center lg:justify-between">
          <div>
            ...
            <p className="mt-1 text-xs text-slate-500 sm:text-sm">
              Manage and review all job openings from one clean dashboard.
            </p>
          </div>
        ...
        <div className="mb-4 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm">
          ...
          <div className="grid grid-cols-1 gap-2 sm:grid-cols-2 lg:grid-cols-4 xl:grid-cols-6">
            ...
            >
              ...
              {(filterOptions?.employmentModes || []).map((item) => (
            ...
            >
              ...
              <option value="applyType">Type</option>
            ...
            <table className="min-w-full">
              ...
              <tbody className="divide-y divide-slate-100">
                {loading || tableLoading ? (
                  ...
                  jobs.map((job) => (
                    <tr key={job._id} className="transition hover:bg-slate-50">
                      ...
                      <td className="px-3 py-3 text-xs font-medium text-slate-700 capitalize">
                        {job.applyType || "-"}
                      </td>
                      ...
                      <td className="px-3 py-3">{getStatusBadge(job)}</td>
                      ...
                      <td className="px-3 py-3">
                        <div className="flex flex-wrap gap-1.5">
                          ...
                          <button
                            onClick={() => doSinglePublishToggle(job._id)}
                          ...
                          <button
                            onClick={() => doSingleSoftDelete(job._id)}
