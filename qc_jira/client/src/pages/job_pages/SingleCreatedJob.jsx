    ...
    try {
      ...
      if (type === "publish") {
        response = await axios.patch(
          `${globalBackendRoute}/api/jobs/${id}/publish`,
          {},
          { headers: authHeaders },
        );
      } else if (type === "unpublish") {
        response = await axios.patch(
          `${globalBackendRoute}/api/jobs/${id}/unpublish`,
          {},
          { headers: authHeaders },
        );
      } else if (type === "restore") {
        response = await axios.patch(
          `${globalBackendRoute}/api/jobs/${id}/restore`,
          {},
          { headers: authHeaders },
        );
        ...
        response = await axios.delete(
          `${globalBackendRoute}/api/jobs/${id}/soft-delete`,
          {
            headers: authHeaders,
          },
        );
        ...
        response = await axios.delete(
          `${globalBackendRoute}/api/jobs/${id}/hard-delete`,
          {
            headers: authHeaders,
          },
        );

        setServerMessage(response?.data?.message || "Job permanently deleted.");
      ...
      console.error("Single job action error:", error);
      setServerError(
      ...
      <div className="mx-auto w-full max-w-[98%]">
          ...
          <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
            <div>
              ...
              <div className="mt-2 flex flex-wrap items-center gap-2">
                ...
                <span className="rounded-full border border-slate-200 bg-slate-50 px-2.5 py-1 text-[11px] font-semibold text-slate-600">
                  {job.preferredLocation || "No location"}
                </span>
              </div>
            </div>
        ...
        <div className="grid grid-cols-1 gap-4 xl:grid-cols-3">
          <div className="space-y-4 xl:col-span-2">
            <div className={sectionCard}>
              ...
              <p className="mt-3 whitespace-pre-wrap text-xs leading-6 text-slate-700">
                {job.shortDescription || "No short description added."}
              </p>
            </div>
            ...
            <div className={sectionCard}>
              ...
              <p className="mt-3 whitespace-pre-wrap text-xs leading-6 text-slate-700">
                {job.description || "No full description added."}
              </p>
            </div>

            {renderArrayList("Responsibilities", job.responsibilities)}
            {renderArrayList("Requirements", job.requirements)}
            {renderArrayList("Benefits", job.benefits)}
          </div>
          ...
          <div className="space-y-4">
            <div className={sectionCard}>
              ...
              <div className="mt-3 space-y-2.5 text-xs text-slate-700">
                <div className="flex justify-between gap-3 border-b border-slate-100 pb-2.5">
                  ...
                  <span className="text-right">{job.slug || "-"}</span>
                </div>
                <div className="flex justify-between gap-3 border-b border-slate-100 pb-2.5">
                  <span className="font-semibold">Type</span>
                  <span className="capitalize">{job.applyType || "-"}</span>
                </div>
                <div className="flex justify-between gap-3 border-b border-slate-100 pb-2.5">
            ...
            <div className={sectionCard}>
              <h2 className="text-sm font-bold text-slate-800">Salary</h2>
              ...
              <div className="mt-3">
                {job.externalApplyLink ? (
                  <a
                    ...
                    target="_blank"
                    rel="noreferrer"
                    className="text-xs font-semibold text-blue-600 hover:text-blue-700 break-all"
                  >
                    {job.externalApplyLink}
                  </a>
                ) : (
                  <p className="text-xs text-slate-500">
                    No external apply link added.
                  </p>
                )}
              </div>
            </div>
