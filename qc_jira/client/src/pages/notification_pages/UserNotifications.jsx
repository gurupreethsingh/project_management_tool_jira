...
export default function UserNotifications() {
  ...
  // --- local "seen" state (per-user, persisted) ---
  ...
  const openNotification = async (n) => {
    ...
    // Optimistically mark as seen locally for instant subtle color change
    markSeenLocal(id);
    try {
      await axios.put(
        `${API}/mark-read/${id}`,
      ...
      <div className="mt-4 divide-y rounded-lg border overflow-hidden">
        {loading ? (
          ...
          pageRows.map((n) => {
            ...
            // Subtle grayscale indicator: seen => light gray bg + thin gray left border
