      ...
      <div className="bg-white shadow rounded-lg p-4 mb-4 border border-slate-200">
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 gap-4 mt-2">
          <div className="bg-sky-50 p-4 rounded-lg border border-sky-100">
            <h3 className="text-md font-semibold mb-2 text-sky-800">
              <FaLaptopCode className="mr-2 inline text-sky-500" />
              Developers
            </h3>
            {project.developers?.length > 0 ? (
              <ul>
                {project.developers.map((dev, idx) => (
                  <li
                    key={dev?._id || `dev-${dev?.name || "unknown"}-${idx}`}
              ...
              <p className="text-slate-600">No developers assigned.</p>
            )}
          </div>

          <div className="bg-emerald-50 p-4 rounded-lg border border-emerald-100">
            <h3 className="text-md font-semibold mb-2 text-emerald-800">
              <FaHardHat className="mr-2 inline text-emerald-500" />
              Test Engineers
            </h3>
            {project.testEngineers?.length > 0 ? (
              <ul>
                {project.testEngineers.map((eng, idx) => (
                  <li
                    key={eng?._id || `eng-${eng?.name || "unknown"}-${idx}`}
                    className="flex items-center text-slate-700"
                  >
              ...
              <p className="text-slate-600">No test engineers assigned.</p>
            )}
          </div>
        </div>
      </div>
